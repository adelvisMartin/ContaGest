...
try {
  for (const volume of volumes) {
    ...
    const recentRows = await client.query(
      `SELECT external_message_id,received_at FROM public.hipico_messages
       WHERE owner_id=$1::uuid AND channel_key=$2 ORDER BY received_at DESC LIMIT 50`,
      [OWNER_ID, groupKey]
    );
    ...
    evidence.push({
      ...
      recent50Ms: Number(recentMs.toFixed(3)),
