...
function lifecycleEvent(type: 'RACE_OPENED' | 'RACE_CLOSED', sourceMessageKey: string, timestamp: string, aggregateKey: string) {
  return {
    type,
    sourceMessageKey,
...
async function eventCount(groupKey: string, aggregateKey: string) {
  const rows = await prisma.$queryRaw<Array<{ count: bigint }>>`
    ...
    WHERE owner_id=${OWNER_ID}::uuid AND group_key=${groupKey}
      AND aggregate_kind='race' AND aggregate_key=${aggregateKey}
  `;
...
try {
  if (phase === 'prepare') {
    ...
    await fs.writeFile(artifact, `${JSON.stringify({ schema: 'hipico-restart.v290', ownerId: OWNER_ID, groupKey, aggregateKey, sourceMessageKey, timestamp, preparedAt: new Date().toISOString() }, null, 2)}\n`, 'utf8');
    ...
    const before = await readHipicoDomainAggregate({ ownerId: OWNER_ID, groupKey: state.groupKey, aggregateKind: 'race', aggregateKey: state.aggregateKey, limit: 20 });
    ...
    const after = await readHipicoDomainAggregate({ ownerId: OWNER_ID, groupKey: state.groupKey, aggregateKind: 'race', aggregateKey: state.aggregateKey, limit: 20 });
    ...
    await fs.writeFile(artifact, `${JSON.stringify({ ...state, verifiedAt: new Date().toISOString(), status: 'PASS', finalState: 'CLOSED' }, null, 2)}\n`, 'utf8');
