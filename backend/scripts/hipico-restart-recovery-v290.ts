...
function cmd(command: any, expectedState: any, requestId: string) {
  return { command, expectedState, requestId, actorId: 'restart-e2e', actorType: 'operator' as const, correlationId: `corr-${requestId}`, payload: {}, evidence: [] };
}
...
try {
  if (phase === 'prepare') {
    ...
    assert.equal(eventsAfterReplay.filter((event: any) => event.requestId === state.openRequestId).length, 1);
    ...
    assert.equal(finalRace?.state, 'CLOSED');
    await fs.writeFile(stateFile, JSON.stringify({ ...state, verifiedAt: new Date().toISOString(), finalState: finalRace?.state, replayBlocked: true, eventCount: (await store.history(OWNER_ID, GROUP_KEY, state.raceId)).length }, null, 2));
    console.log(`[hipico-v290] restart recovery verified for race ${state.raceId}`);
  } else throw new Error('Usage: tsx scripts/hipico-restart-recovery-v290.ts <prepare|verify>');
