...
import {ensureAccountMembership} from '../identity/accountMembership.js';
...
async function assignSubscriptionRole(tenantId:string,userId:string,modules:string[]){
  ...
  const role=await prisma.role.upsert({
    ...
    update:{description:'Acceso derivado de suscripción comercial.',system:false},
    create:{tenantId,name:roleName,description:'Acceso derivado de suscripción comercial.',system:false}
  });
...
export async function provisionSubscriptionAccess(input:{subscriptionId:string;tenantId:string;email:string;fullName:string;maxDevices?:number}){
  ...
  const subscriptionRows=await prisma.$queryRaw<Array<{id:string;planCode:string;billingCycle:string;maxUsers:number;customerSegment:string}>>`
    SELECT s."id",s."planCode",s."billingCycle",s."maxUsers",s."customerSegment"
    FROM public."Subscription" s
    JOIN public."SubscriptionTenant" st ON st."subscriptionId"=s."id"
    WHERE s."id"=${input.subscriptionId} AND st."tenantId"=${input.tenantId} AND st."status"='active'
    LIMIT 1
  `;
  const subscription=subscriptionRows[0];
  if(!subscription)throw new HttpError(404,'La empresa no forma parte de la suscripción.');
  const tenant=await prisma.tenant.findUnique({where:{id:input.tenantId},select:{id:true,rif:true,name:true}});
  if(!tenant)throw new HttpError(404,'Empresa no encontrada.');
  const entitlements=await prisma.$queryRaw<Array<{moduleCode:string;kind:string}>>`
    SELECT "moduleCode","kind" FROM public."ModuleEntitlement"
    WHERE "subscriptionId"=${input.subscriptionId} AND "status"='active'
    ORDER BY "kind","moduleCode"
  `;
  const modules=entitlements.map(item=>item.moduleCode).filter(code=>!code.startsWith('channel.'));
  if(!modules.length)throw new HttpError(422,'La suscripción no tiene módulos operativos habilitados.');

  const target=await prisma.userProfile.findUnique({where:{tenantId_email:{tenantId:tenant.id,email}}});
  const source=target?.passwordHash?target:await prisma.userProfile.findFirst({where:{email,status:'active',passwordHash:{not:null}},orderBy:{createdAt:'asc'}});
  ...
  const user=target
    ? await prisma.userProfile.update({where:{id:target.id},data:{fullName:input.fullName,passwordHash,status:'active'}})
    : await prisma.userProfile.create({data:{tenantId:tenant.id,email,fullName:input.fullName,passwordHash,status:'active'}});

  const membership=await ensureAccountMembership({tenantId:tenant.id,userProfileId:user.id,email:user.email,fullName:user.fullName,roleLabel:subscription.planCode==='contador'?'Contador':'Cliente suscrito'});
  await assignSubscriptionRole(tenant.id,user.id,modules);

  const rawKey=generateLicenseKey();
  const technicalExpiry=new Date(Date.now()+3650*86400000);
  const config={enabled:modules,businessSector:subscription.customerSegment||'general',commercialUse:'operacion'};
  let record;
  try{
    ...
    await prisma.$executeRaw`
      ...
      SET "businessCategory"=${subscription.customerSegment||'general'},"companyName"=${tenant.name},"companyRif"=${tenant.rif},
          "maxUsers"=1,"maxDevices"=${Math.min(20,Math.max(1,Number(input.maxDevices||2)))},"activationCount"=0,
          "subscriptionId"=${input.subscriptionId},"issuedForMembershipId"=${membership?.id||null},"updatedAt"=now()
      WHERE "id"=${record.id}
    `;
  }catch(error:any){
  ...
  if(previous.length){
    ...
    for(const item of previous)await prisma.$executeRaw`UPDATE public."LicenseActivation" SET "status"='revoked',"revokedAt"=now() WHERE "licenseId"=${item.id}`;
  }

  return{
    licenseId:record.id,subscriptionId:input.subscriptionId,tenant:{id:tenant.id,rif:tenant.rif,name:tenant.name},
    user:{id:user.id,email:user.email,fullName:user.fullName},membershipId:membership?.id||null,modules,maxDevices:Math.min(20,Math.max(1,Number(input.maxDevices||2))),
    licenseKey:rawKey,temporaryPassword,technicalExpiresAt:technicalExpiry.toISOString(),
    note:temporaryPassword?'Guarda la contraseña y licencia ahora; se muestran una sola vez.':'La identidad ya existía. Conserva su contraseña actual; guarda únicamente la nueva licencia técnica.'
  };
}
