...
router.post('/periods', validate(periodSchema), asyncHandler(async (req,res)=>{
  const ctx=context(req);
  ...
  await writeAudit({ tenantId:ctx.tenantId,userId:ctx.userId,module:'payroll',action:'create-period',entity:'PayrollPeriod',entityId:created.id,after:serializePeriod(created),ip:ctx.ip,userAgent:ctx.userAgent });
  ok(res,serializePeriod(created));
}));

router.post('/receipts', validate(receiptSchema), asyncHandler(async (req,res)=>{
  const ctx=context(req);
  ...
  const created=await prisma.payrollReceipt.create({
    data:{ periodId:period.id,employeeId:employee.id,gross:input.gross,deductions:input.deductions,net:input.net,details:input.details || {} },
    include:{ employee:true }
  });
  const updatedPeriod=await recalculatePeriod(period.id);
  await writeAudit({ tenantId:ctx.tenantId,userId:ctx.userId,module:'payroll',action:'create-receipt',entity:'PayrollReceipt',entityId:created.id,after:serializeReceipt(created),ip:ctx.ip,userAgent:ctx.userAgent });
  ok(res,{ receipt:serializeReceipt(created),period:serializePeriod(updatedPeriod) });
}));

router.patch('/periods/:id/status', validate(statusSchema), asyncHandler(async (req,res)=>{
  const ctx=context(req);
  ...
  await writeAudit({ tenantId:ctx.tenantId,userId:ctx.userId,module:'payroll',action:`period-${req.body.status}`,entity:'PayrollPeriod',entityId:updated.id,before:serializePeriod(existing),after:serializePeriod(updated),ip:ctx.ip,userAgent:ctx.userAgent });
...
router.delete('/receipts/:id', asyncHandler(async (req,res)=>{
  ...
  await writeAudit({ tenantId:ctx.tenantId,userId:ctx.userId,module:'payroll',action:'delete-receipt',entity:'PayrollReceipt',entityId:existing.id,before:serializeReceipt(existing),ip:ctx.ip,userAgent:ctx.userAgent });
