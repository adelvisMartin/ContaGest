...
const updateSchema=z.object({name:z.string().trim().min(2).max(180).optional(),legalName:z.string().trim().max(220).nullable().optional(),settings:z.record(z.string(),z.unknown()).optional()}).strict();
...
router.get('/',asyncHandler(async(req,res)=>{
  const ctx=(req as any).context;const data=await prisma.tenant.findUnique({where:{id:ctx.tenantId}});if(!data)throw new HttpError(404,'Empresa no encontrada.');ok(res,[data]);
}));
router.get('/:id',asyncHandler(async(req,res)=>{assertCurrentTenant(req,req.params.id);const data=await prisma.tenant.findUnique({where:{id:req.params.id}});if(!data)throw new HttpError(404,'Empresa no encontrada.');ok(res,data);}));
router.put('/:id',asyncHandler(async(req,res)=>{
  ...
  const body=updateSchema.parse(req.body||{});const ctx=(req as any).context;
  const before=await prisma.tenant.findUnique({where:{id:req.params.id}});if(!before)throw new HttpError(404,'Empresa no encontrada.');
  const data=await prisma.tenant.update({where:{id:req.params.id},data:body});
  await writeAudit({tenantId:ctx.tenantId,userId:ctx.userId,action:'tenant.update.safe',entity:'tenant',entityId:data.id,before,after:data,ipAddress:ctx.ip,userAgent:ctx.userAgent});ok(res,data);
}));
