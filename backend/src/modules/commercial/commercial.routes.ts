...
const subscriptionSchema=z.object({
  ...
  currency:z.string().trim().min(3).max(8).default('USD'),amount:z.coerce.number().min(0).default(0),status:z.enum(['trial','active','past_due','suspended','cancelled','expired']).default('trial'),
...
const patchSubscriptionSchema=z.object({
  status:z.enum(['trial','active','past_due','suspended','cancelled','expired']).optional(),amount:z.coerce.number().min(0).optional(),nextRenewalAt:z.coerce.date().nullable().optional(),graceUntil:z.coerce.date().nullable().optional(),
...
router.patch('/subscriptions/:id',asyncHandler(async(req,res)=>{const b=patchSubscriptionSchema.parse(req.body||{});const current=await subscriptionRow(req.params.id);if(!current)throw new HttpError(404,'Suscripción no encontrada.');const next={status:b.status??current.status,amount:b.amount??Number(current.amount),nextRenewalAt:b.nextRenewalAt===undefined?current.nextRenewalAt:b.nextRenewalAt,graceUntil:b.graceUntil===undefined?current.graceUntil:b.graceUntil,maxTenants:b.maxTenants??Number(current.maxTenants),maxUsers:b.maxUsers??Number(current.maxUsers),supportLevel:b.supportLevel??current.supportLevel};await prisma.$executeRaw`UPDATE public."Subscription" SET "status"=${next.status},"amount"=${next.amount},"nextRenewalAt"=${next.nextRenewalAt},"graceUntil"=${next.graceUntil},"maxTenants"=${next.maxTenants},"maxUsers"=${next.maxUsers},"supportLevel"=${next.supportLevel},"updatedAt"=now() WHERE "id"=${req.params.id}`;const result=await subscriptionRow(req.params.id);await audit(req,'commercial.subscription.update','Subscription',req.params.id,result);ok(res,result);}));
...
router.post('/payments',asyncHandler(async(req,res)=>{const b=paymentSchema.parse(req.body||{});const subscription=await subscriptionRow(b.subscriptionId);if(!subscription)throw new HttpError(404,'Suscripción no encontrada.');const paidAt=b.status==='paid'?(b.paidAt||new Date()):null;const periodStart=b.periodStart||(b.status==='paid'?new Date():null);const periodEnd=b.periodEnd||(periodStart&&subscription.billingCycle!=='manual'?addCycle(new Date(periodStart),subscription.billingCycle):null);const rows=await prisma.$queryRaw<any[]>`INSERT INTO public."SubscriptionPayment" ("id","subscriptionId","amount","currency","method","reference","status","periodStart","periodEnd","paidAt","metadata","createdAt","updatedAt") VALUES (gen_random_uuid()::text,${b.subscriptionId},${b.amount},${b.currency.toUpperCase()},${b.method||null},${b.reference||null},${b.status},${periodStart},${periodEnd},${paidAt},'{}'::jsonb,now(),now()) RETURNING *`;const payment=rows[0];if(b.status==='paid'){const next=periodEnd||addCycle(new Date(paidAt!),subscription.billingCycle);await prisma.$executeRaw`UPDATE public."Subscription" SET "status"='active',"currentPeriodStart"=${periodStart},"currentPeriodEnd"=${periodEnd},"nextRenewalAt"=${next},"updatedAt"=now() WHERE "id"=${b.subscriptionId}`;await prisma.$executeRaw`UPDATE public."CustomerAccount" SET "status"='active',"updatedAt"=now() WHERE "id"=${subscription.customerAccountId}`;if(subscription.salesAgentId){const existing=await prisma.$queryRaw<Array<{id:string}>>`SELECT "id" FROM public."Commission" WHERE "paymentId"=${payment.id} LIMIT 1`;if(!existing[0]){const rate=Number(subscription.commissionRate||0),commissionAmount=Math.round((b.amount*rate/100)*100)/100;await prisma.$executeRaw`INSERT INTO public."Commission" ("id","salesAgentId","subscriptionId","paymentId","rate","baseAmount","amount","currency","status","earnedAt","createdAt","updatedAt") VALUES (gen_random_uuid()::text,${subscription.salesAgentId},${b.subscriptionId},${payment.id},${rate},${b.amount},${commissionAmount},${b.currency.toUpperCase()},'earned',now(),now(),now())`;}}}await audit(req,'commercial.payment.create','SubscriptionPayment',payment.id,payment);ok(res,{payment,subscription:await subscriptionRow(b.subscriptionId)},201);}));
