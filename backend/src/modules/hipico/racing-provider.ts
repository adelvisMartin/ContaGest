...
export function freshnessFrom(sourceTimestamp:string|null,fetchedAt:string,now=Date.now()):RacingFreshness{const source=sourceTimestamp?Date.parse(sourceTimestamp):NaN,fetched=Date.parse(fetchedAt),basis=Number.isFinite(source)?source:fetched;if(!Number.isFinite(basis))return'OFFLINE';if(basis>now+MAX_PROVIDER_CLOCK_SKEW_MS)return'OFFLINE';const age=Math.max(0,now-basis);if(age<=30_000)return'LIVE';if(age<=5*60_000)return'FRESH';if(age<=60*60_000)return'STALE';return'OFFLINE';}
