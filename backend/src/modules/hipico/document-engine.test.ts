...
class MemoryStore implements DocumentStore{
  ...
  async put(input:DocumentStoreInput){const prior=this.hashes.get(input.envelope.sha256);if(prior){const row=this.rows.get(prior);return{id:prior,duplicate:true,status:row?.status||'uploaded'};}const id=`00000000-0000-4000-8000-${String(this.rows.size+1).padStart(12,'0')}`;this.hashes.set(input.envelope.sha256,id);this.rows.set(id,{...input,status:'uploaded'});return{id,duplicate:false,status:'uploaded'};}
  async claimExtraction(id:string,_owner:string,_group:string,allowProcessed=false){const row=this.rows.get(id);if(!row||row.extraction?.status==='processing')return false;if(!allowProcessed&&!['uploaded','failed'].includes(row.status))return false;row.status='uploaded';row.extraction={status:'processing'};if(allowProcessed){row.approvedAt=null;row.approvedBy=null;}return true;}
  async markExtractionFailed(id:string,_owner:string,_group:string,errorCode:string){const row=this.rows.get(id);if(!row)throw new Error('HIPICO_DOCUMENT_NOT_FOUND');row.status='failed';row.extraction={status:'failed',errorCode};}
  async updateExtraction(id:string,_owner:string,_group:string,input:any){const row=this.rows.get(id);if(!row)throw new Error('HIPICO_DOCUMENT_NOT_FOUND');this.updates.push({id,...input});row.status=input.status;row.extraction=input.extraction;row.classification=input.classification;}
