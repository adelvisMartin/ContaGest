...
function sendError(req: Request, res: Response, error: any) {
  ...
  return res.status(status).json(hipicoError({
    code,
    message: 'No se pudo construir el Centro de Operaciones de Control Hípico.',
...
router.get('/command-center', async (req, res) => {
  try { return res.status(200).json({ ok: true, data: await buildHipicoCommandCenter({ ownerId: ownerId(), groupKey: groupKey(req) }) }); }
