...
export class PostgresDocumentStore implements DocumentStore {
  async put(input:DocumentStoreInput){
    ...
    return prisma.$transaction(async(tx)=>{
      ...
      const existing=await tx.$queryRaw<Array<{id:string}>>`SELECT id FROM public.hipico_documents WHERE owner_id=${input.ownerId}::uuid AND group_key=${input.groupKey} AND sha256=${input.envelope.sha256} LIMIT 1 FOR UPDATE`;
