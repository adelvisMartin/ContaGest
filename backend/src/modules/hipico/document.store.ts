...
function assertScope(ownerId:string,groupKey:string){
  if(!ID_RE.test(ownerId))throw Object.assign(new Error('HIPICO_DOCUMENT_OWNER_INVALID'),{code:'HIPICO_DOCUMENT_OWNER_INVALID'});
  if(!GROUP_KEY_RE.test(groupKey))throw Object.assign(new Error('HIPICO_DOCUMENT_GROUP_INVALID'),{code:'HIPICO_DOCUMENT_GROUP_INVALID'});
}
...
export class PostgresDocumentStore implements DocumentStore {
    ...
    return prisma.$transaction(async(tx)=>{
      if(input.supersedesId){
        const prior=await tx.$queryRaw<Array<{id:string}>>`
          ...
          LIMIT 1 FOR UPDATE`;
        if(!prior[0])throw Object.assign(new Error('HIPICO_DOCUMENT_SUPERSEDES_NOT_FOUND'),{code:'HIPICO_DOCUMENT_SUPERSEDES_NOT_FOUND'});
      }
      ...
      if(input.provenance.sourceMessageId){
        const replay=await tx.$queryRaw<Array<{documentId:string;sha256:string}>>`
          ...
          LIMIT 1`;
        if(replay[0]&&replay[0].sha256!==input.envelope.sha256){
          throw Object.assign(new Error('HIPICO_DOCUMENT_SOURCE_REPLAY_MISMATCH'),{code:'HIPICO_DOCUMENT_SOURCE_REPLAY_MISMATCH'});
        }
      ...
      const existing=await tx.$queryRaw<Array<{id:string}>>`
        ...
        LIMIT 1 FOR UPDATE`;
      let documentId=existing[0]?.id||crypto.randomUUID();
      if(!existing[0]){
        await tx.$executeRaw`
  ...
  async updateExtraction(id:string,ownerId:string,groupKey:string,input:{classification:DocumentClassification;confidence:number;parserVersion:string;status:'extracted'|'review';extraction:Record<string,unknown>}){
    ...
    const affected=await prisma.$executeRaw`
      ...
      SET classification=${input.classification},confidence=${input.confidence},parser_version=${input.parserVersion},
    ...
    if(affected!==1)throw Object.assign(new Error('HIPICO_DOCUMENT_NOT_FOUND'),{code:'HIPICO_DOCUMENT_NOT_FOUND'});
  }
  ...
  async approve(ownerId:string,groupKey:string,id:string,classification:DocumentClassification,operatorId:string){
    ...
    return prisma.$transaction(async(tx)=>{
      ...
      if(!rows[0])throw Object.assign(new Error('HIPICO_DOCUMENT_NOT_FOUND'),{code:'HIPICO_DOCUMENT_NOT_FOUND'});
      if(classification==='OFFICIAL_RESULT'&&rows[0].authority!=='official'){
        throw Object.assign(new Error('HIPICO_DOCUMENT_OFFICIAL_AUTHORITY_REQUIRED'),{code:'HIPICO_DOCUMENT_OFFICIAL_AUTHORITY_REQUIRED'});
      }
      await tx.$executeRaw`
        UPDATE public.hipico_documents
        SET classification=${classification},status='approved',approved_at=now(),approved_by=${operatorId},updated_at=now()
        WHERE id=${id}::uuid AND owner_id=${ownerId}::uuid AND group_key=${groupKey}`;
      return{ok:true};
    });
  }
