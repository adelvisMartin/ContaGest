...
const READ_ONLY_QUERY_TOOLS = new Set<AgentTool>([
  'queryRaceStatus',
  'queryNextRace',
  'queryLastResult',
  'querySchedule',
...
const MIN_AUTOMATIC_CONFIDENCE = .95;
...
export function agentCanAct(mode: AutomationState, candidate: AgentCandidate) {
  ...
  if (candidate.confidence < MIN_AUTOMATIC_CONFIDENCE) return false;
  if (!candidate.tool || !READ_ONLY_QUERY_TOOLS.has(candidate.tool)) return false;
  return true;
}
...
const QUERY_TOOLS = READ_ONLY_QUERY_TOOLS;
