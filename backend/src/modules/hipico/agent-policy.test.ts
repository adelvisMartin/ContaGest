...
void test('automatic execution requires high confidence and a read-only query tool', () => {
  const safe = { intent: 'query', confidence: .99, tool: 'queryRaceStatus' as const, arguments: {}, risk: 'safe' as const, source: 'deterministic' as const, modelVersion: null };
  assert.equal(agentCanAct('AUTOMATIC_LOW_RISK', { ...safe, confidence: .949 }), false);
  assert.equal(agentCanAct('AUTOMATIC', { ...safe, tool: null }), false);
  assert.equal(agentCanAct('AUTOMATIC', { ...safe, tool: 'proposeRaceCommand' }), false);
});
