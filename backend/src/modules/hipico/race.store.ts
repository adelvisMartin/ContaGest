...
function scope(ownerId:string,groupKey:string){if(!UUID_RE.test(ownerId))throw new Error('HIPICO_OWNER_INVALID');if(!GROUP_RE.test(groupKey))throw new Error('HIPICO_GROUP_INVALID');}
function stable(value:unknown):string{
  if(value===null||value===undefined)return 'null';
  if(Array.isArray(value))return`[${value.map(stable).join(',')}]`;
  if(typeof value==='object'){const row=value as Record<string,unknown>;return`{${Object.keys(row).sort().map((key)=>`${JSON.stringify(key)}:${stable(row[key])}`).join(',')}}`;}
  return JSON.stringify(value);
}
function signature(input:RaceCommandInput){return crypto.createHash('sha256').update(stable(input)).digest('hex');}
...
export class RaceLifecycleStore {
  async createMeeting(input:{ownerId:string;groupKey:string;name:string;meetingDate?:string|null;venue?:string|null;externalRef?:string|null}){
    scope(input.ownerId,input.groupKey);const id=crypto.randomUUID();
    const rows=await prisma.$queryRaw<any[]>`
      ...
      RETURNING id,name,meeting_date AS "meetingDate",venue,external_ref AS "externalRef",created_at AS "createdAt"`;
    return rows[0];
  }
  async listMeetings(ownerId:string,groupKey:string,limit=50){
    scope(ownerId,groupKey);const bounded=Math.min(100,Math.max(1,Math.trunc(limit)||50));
    return prisma.$queryRaw<any[]>`SELECT id,name,meeting_date AS "meetingDate",venue,external_ref AS "externalRef",created_at AS "createdAt" FROM public.hipico_meetings WHERE owner_id=${ownerId}::uuid AND group_key=${groupKey} ORDER BY meeting_date DESC NULLS LAST,created_at DESC LIMIT ${bounded}`;
  }
  async getMeeting(ownerId:string,groupKey:string,id:string){
    scope(ownerId,groupKey);const rows=await prisma.$queryRaw<any[]>`SELECT id,name,meeting_date AS "meetingDate",venue,external_ref AS "externalRef",created_at AS "createdAt" FROM public.hipico_meetings WHERE id=${id}::uuid AND owner_id=${ownerId}::uuid AND group_key=${groupKey} LIMIT 1`;return rows[0]||null;
  }
  ...
  async listRaces(ownerId:string,groupKey:string,meetingId?:string|null,limit=100){
    scope(ownerId,groupKey);const bounded=Math.min(200,Math.max(1,Math.trunc(limit)||100));
    if(meetingId)return prisma.$queryRaw<any[]>`SELECT id,meeting_id AS "meetingId",race_number AS "number",name,scheduled_at AS "scheduledAt",external_ref AS "externalRef",state,result_stage AS "resultStage",state_version AS "stateVersion",result_data AS "resultData" FROM public.hipico_races WHERE owner_id=${ownerId}::uuid AND group_key=${groupKey} AND meeting_id=${meetingId}::uuid ORDER BY race_number ASC LIMIT ${bounded}`;
    return prisma.$queryRaw<any[]>`SELECT id,meeting_id AS "meetingId",race_number AS "number",name,scheduled_at AS "scheduledAt",external_ref AS "externalRef",state,result_stage AS "resultStage",state_version AS "stateVersion",result_data AS "resultData" FROM public.hipico_races WHERE owner_id=${ownerId}::uuid AND group_key=${groupKey} ORDER BY scheduled_at DESC NULLS LAST,race_number ASC LIMIT ${bounded}`;
  }
  async getRace(ownerId:string,groupKey:string,id:string){
