...
router.post('/races/:id/commands',async(req,res)=>{
  try{
    const body=commandSchema.parse(req.body),headerKey=String(req.header('idempotency-key')||'').trim();
    ...
    const input:RaceCommandInput={...body,idempotencyKey,actorType:'operator'};
