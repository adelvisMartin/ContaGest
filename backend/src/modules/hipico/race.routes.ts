...
function status(code:string){if(code.includes('NOT_FOUND'))return 404;if(code.includes('IDEMPOTENCY')||code==='RACE_STATE_CONFLICT'||code==='EXPECTED_STATE_MISMATCH'||code==='INVALID_TRANSITION'||code.includes('REQUIRES_')||code.startsWith('RACE_QUERY_'))return 409;if(code==='HIPICO_OWNER_NOT_CONFIGURED')return 503;return 400;}
...
function queryError(code:string){throw Object.assign(new Error(code),{code});}
...
function exactRaceContext(races:any[],intent:RaceQueryIntent,raceId:string|null){
  if(raceId){const match=races.find((race:any)=>race.id===raceId);if(!match)queryError('HIPICO_RACE_NOT_FOUND');return match;}
  const resultIntent=intent==='RESULT'||intent==='OFFICIALITY';
  const candidates=resultIntent
    ?races.filter((race:any)=>['PROVISIONAL_RESULT','OFFICIAL_RESULT','ARCHIVED'].includes(race.state)||race.resultStage!=='none')
    :races.filter((race:any)=>['OPEN','CLOSING','RUNNING'].includes(race.state));
...
router.get('/races',async(req,res)=>{try{const meetingId=optionalUuid(req.query.meetingId);return res.json({ok:true,data:await store.listRaces(ownerId(),groupKey(req),meetingId,Number(req.query.limit)||100)});}catch(error){return sendError(req,res,error);}});
...
router.get('/queries/races',async(req,res)=>{
  try{
    const text=z.string().trim().min(1).max(500).parse(req.query.text);
    const intent=classifyRaceQueryIntent(text);
    const owner=ownerId(),g=groupKey(req),meetingId=optionalUuid(req.query.meetingId),raceId=optionalUuid(req.query.raceId);
    const races=await store.listRaces(owner,g,meetingId,200);
    let answer:any=null;
    if(intent==='ACTIVE_RACE'){
      ...
      const candidates=[...races].filter((race:any)=>race.state==='DISCOVERED'||race.state==='ANNOUNCED').sort((a:any,b:any)=>Date.parse(a.scheduledAt||'9999-12-31')-Date.parse(b.scheduledAt||'9999-12-31')||a.number-b.number);
      if(candidates.length>1&&String(candidates[0].scheduledAt||'')===String(candidates[1].scheduledAt||'')&&candidates[0].meetingId!==candidates[1].meetingId)queryError('RACE_QUERY_AMBIGUOUS');
      answer=candidates[0]||null;
      ...
      if(meetingId){const meeting=await store.getMeeting(owner,g,meetingId);if(!meeting)queryError('HIPICO_MEETING_NOT_FOUND');answer={...meeting,races};}
      else{const meetings=await store.listMeetings(owner,g,20);if(meetings.length!==1)queryError(meetings.length?'RACE_QUERY_AMBIGUOUS':'RACE_QUERY_CONTEXT_REQUIRED');answer={...meetings[0],races:await store.listRaces(owner,g,meetings[0].id,200)};}
      ...
      const data=race.resultData&&typeof race.resultData==='object'?race.resultData:{};
