...
import { documentExtractorCapability } from '../hipico-bot/hipico-document-extractor.js';
import {
  HIPICO_API_VERSION,
...
const text = (source: RuntimeEnv, name: string) => String(source[name] || '').trim();
const strongSecret = (value: string) => Buffer.byteLength(String(value || '').trim(), 'utf8') >= 32;
const whatsappGroupId = (value: string) => /^\d{5,}(?:-\d+)?@g\.us$/i.test(String(value || '').trim());
...
export async function buildHipicoSystemStatus(options: Dependencies = {}): Promise<HipicoSystemStatus> {
  ...
  const groupsPinned = whatsappGroupId(sourceGroupId) && whatsappGroupId(labGroupId) && sourceGroupId.toLowerCase() !== labGroupId.toLowerCase();
  ...
  return hipicoSystemStatusSchema.parse({
    ...
    components: {
      ...
      database: componentState(databaseReady ? 'ready' : readiness.database === 'not_checked' ? 'not_configured' : 'unavailable', databaseReady ? null : readiness.database === 'not_checked' ? 'DATABASE_NOT_CONFIGURED' : 'DATABASE_PROBE_FAILED'),
      bridge: componentState(bridgeConfigured ? 'ready' : 'not_configured', bridgeConfigured ? null : !bridgeSecretReady ? 'BRIDGE_SECRET_NOT_READY' : 'BRIDGE_GROUPS_NOT_PINNED'),
      channel: componentState(groupsPinned ? 'ready' : 'not_configured', groupsPinned ? null : 'SOURCE_LAB_GROUPS_NOT_PINNED'),
      providers: { ...componentState(provider.configured ? 'ready' : 'not_configured', provider.reason), financialAuthority: false },
      documentEngine: componentState(documentDegraded ? 'degraded' : documentReady ? 'ready' : 'not_configured', documentDegraded ? (documentCapability.reason || 'DOCUMENT_OCR_NOT_READY') : documentReady ? null : (documentCapability.reason || 'DOCUMENT_ENGINE_NOT_INSTALLED')),
      agent: componentState('ready', null)
    }
  });
}
...
export function hipicoReadinessFromStatus(status: HipicoSystemStatus) {
  return {
    ...
    checks: { backend: status.components.backend.state, database: status.components.database.state },
