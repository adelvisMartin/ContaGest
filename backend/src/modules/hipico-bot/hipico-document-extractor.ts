...
import { MAX_PDF_PAGES, type PdfTextExtractor, type ExtractionResult } from './hipico-document-engine.js';
...
async function pageCount(pdfPath: string, signal: AbortSignal) {
  ...
  if (!Number.isInteger(pages) || pages < 1) throw Object.assign(new Error('HIPICO_DOCUMENT_PAGE_COUNT_FAILED'), { code: 'HIPICO_DOCUMENT_PAGE_COUNT_FAILED' });
  if (pages > MAX_PDF_PAGES) throw Object.assign(new Error('PDF_PAGE_LIMIT_EXCEEDED'), { code: 'PDF_PAGE_LIMIT_EXCEEDED' });
  return pages;
}
...
async function ocrPdf(pdfPath: string, pages: number, language: string, directory: string, signal: AbortSignal) {
  if (pages > OCR_MAX_PAGES) throw Object.assign(new Error('HIPICO_DOCUMENT_OCR_PAGE_LIMIT'), { code: 'HIPICO_DOCUMENT_OCR_PAGE_LIMIT' });
  const output: string[] = [];
...
export function createPdfDocumentExtractor(source: RuntimeEnv = process.env): PdfTextExtractor | null {
  const capability = documentExtractorCapability(source);
  if (!capability.configured) return null;
  return {
    capability: () => ({ configured: capability.configured, nativeText: capability.nativeText, ocr: capability.ocr, parserVersion: capability.parserVersion, reason: capability.reason }),
    async extract(pdf: Buffer, signal: AbortSignal): Promise<ExtractionResult> {
      ...
      try {
        ...
        if (!capability.ocr) throw Object.assign(new Error('HIPICO_DOCUMENT_OCR_NOT_CONFIGURED'), { code: 'HIPICO_DOCUMENT_OCR_NOT_CONFIGURED' });
        const language = ocrLanguage(source);
        const extracted = await ocrPdf(pdfPath, pages, language, directory, signal);
        if (!extracted.trim()) throw Object.assign(new Error('HIPICO_DOCUMENT_OCR_EMPTY'), { code: 'HIPICO_DOCUMENT_OCR_EMPTY' });
        return {
          text: extracted,
          method: 'ocr',
