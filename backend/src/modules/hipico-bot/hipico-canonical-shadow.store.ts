...
function eventType(intent:string){
  const map:Record<string,string>={
    offer_player:'offer',offer_receiver:'counteroffer',offer_confirmation:'confirmation',pending_confirmation:'confirmation',
    race_open:'race_open',race_close:'race_close',day_close:'day_close',race_result:'result',plan_snapshot:'plan_snapshot',
    settlement_snapshot:'settlement_snapshot',balance_snapshot:'balance_snapshot',cancel_or_correction:'manual_review',
    polla_or_parley:'manual_review',betting_or_balance:'manual_review'
  };
  return map[intent]||'other';
}

function raceKey(result:IntentResult){return operationalRaceContextKey(result.entities);}
function amountOf(result:IntentResult){const value=Number(result.entities?.amount);return Number.isFinite(value)?value:null;}

function canonicalInputSignature(input:CanonicalPersistInput){
  return canonicalReplaySignature({sender:input.sender,sentAt:input.sentAt,messageType:input.messageType||'text',body:input.body,quotedExternalMessageId:input.quotedExternalMessageId});
}
...
async function readCanonicalSource(ownerId:string,groupKey:string,providerMessageId:string){
  const rows=await prisma.$queryRaw<PersistedCanonicalSource[]>`
    SELECT id::text AS "id", sender_id AS "sender", sent_at AS "sentAt", message_type AS "messageType",
      raw_text AS "body", quoted_external_message_id AS "quotedExternalMessageId"
    FROM public.hipico_messages
    WHERE owner_id=${ownerId}::uuid AND channel_key=${groupKey} AND external_message_id=${providerMessageId}
    LIMIT 1
  `;
  return rows[0]||null;
}

async function immutableCanonicalMessage(channel:CanonicalChannel,input:CanonicalPersistInput,fingerprint:string,normalized:Record<string,unknown>,metadata:Record<string,unknown>,sentAt:Date|null){
  const messageRows=await prisma.$queryRaw<Array<{id:string}>>`
    INSERT INTO public.hipico_messages
      (owner_id,channel_id,channel_key,external_message_id,fingerprint,sender_id,sender_label,sender_role,
       quoted_external_message_id,sent_at,message_type,raw_text,classification,confidence,processing_status,normalized,metadata)
    VALUES
    ...
    DO NOTHING RETURNING id::text AS "id"
  `;
...
async function immutableOperationEvent(channel:CanonicalChannel,input:CanonicalPersistInput,messageId:string,normalized:Record<string,unknown>){
  const opType=eventType(input.result.intent);
  const eventKey=`shadow:${input.providerMessageId}`;
  const opPayload={shadow:true,source:'whatsapp-web-bridge',channelRole:input.channelRole,historySync:Boolean(input.historySync),intent:input.result.intent,risk:input.result.risk,reason:input.result.reason,entities:normalized,suggestion:input.result.suggestion,autoEligible:false};
  const inserted=await prisma.$queryRaw<Array<{id:string}>>`
    INSERT INTO public.hipico_operation_events
      (owner_id,group_key,source_message_id,event_key,event_type,event_state,race_key,participant_code,product_type,amount,currency,confidence,payload)
    VALUES
      (${channel.ownerId}::uuid,${channel.groupKey},${messageId}::uuid,${eventKey},${opType},'pending',${raceKey(input.result)},
       ${input.sender||null},${input.result.entities?.play||null},${amountOf(input.result)},${null},${Number(input.result.confidence||0)},${JSON.stringify(opPayload)}::jsonb)
    ON CONFLICT (owner_id,event_key) DO NOTHING RETURNING id::text AS "id"
  `;
  if(inserted[0]?.id)return inserted[0].id;
  const existing=await prisma.$queryRaw<Array<{id:string}>>`
    SELECT id::text AS "id" FROM public.hipico_operation_events
    WHERE owner_id=${channel.ownerId}::uuid AND event_key=${eventKey} LIMIT 1
  `;
...
export async function persistCanonicalShadow(input:CanonicalPersistInput){
  ...
  const metadata={source:'official_web_playwright',mode:'shadow_only',bridgeVersion:input.bridgeVersion,transportEventId:input.transportEventId,channelRole:input.channelRole,labChannelKey:input.labChannelKey||null,historySync:Boolean(input.historySync),mediaKind:input.mediaKind||'none',mediaName:input.mediaName||null,risk:input.result.risk,reason:input.result.reason,fromMe:input.fromMe,rawMeta:input.rawMeta};
  ...
  if(OPERATIONAL_INTENTS.has(input.result.intent))operationEventId=await immutableOperationEvent(channel,input,messageId,normalized);
  ...
  const prediction={shadow:true,channelRole:input.channelRole,historySync:Boolean(input.historySync),intent:input.result.intent,risk:input.result.risk,confidence:input.result.confidence,reason:input.result.reason,entities:normalized,operationEventId};
  const notes=input.historySync?'Prediccion historica del grupo oficial; solo dataset shadow, sin respuesta LAB ni efecto operativo.':input.channelRole==='source'?'Prediccion del grupo oficial para validacion en laboratorio; sin efecto operativo.':'Prediccion generada en laboratorio; sin efecto operativo.';
  const shadowRows=await prisma.$queryRaw<Array<{id:string}>>`
    INSERT INTO public.hipico_shadow_evaluations
      (owner_id,source_group_key,lab_group_key,source_message_id,source_external_message_id,scenario_key,prediction_type,predicted_payload,match_status,notes)
    VALUES
      (${channel.ownerId}::uuid,${channel.groupKey},${labGroupKey},${messageId}::uuid,${input.providerMessageId},${scenarioKey},${PREDICTION_TYPE},${JSON.stringify(prediction)}::jsonb,'pending',${notes})
    ON CONFLICT (owner_id,source_group_key,source_external_message_id,prediction_type)
    DO UPDATE SET source_message_id=EXCLUDED.source_message_id,lab_group_key=EXCLUDED.lab_group_key,scenario_key=EXCLUDED.scenario_key,
      predicted_payload=EXCLUDED.predicted_payload,predicted_at=now(),match_status='pending',notes=EXCLUDED.notes
    RETURNING id::text AS "id"
  `;

  return{channelId:channel.id,groupKey:channel.groupKey,channelRole:input.channelRole,historySync:Boolean(input.historySync),labGroupKey,messageId,operationEventId,shadowEvaluationId:shadowRows[0]?.id||null,sourceInserted:message.inserted};
}
