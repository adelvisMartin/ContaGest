...
import { z } from 'zod';
import { HipicoBotStore, promotion } from './hipico-bot.service.js';
...
router.get('/status',async(_req,res)=>res.json({ok:true,data:{
  ...
  canonicalOutbox:await canonicalOutboxReadiness(),
...
router.get('/outbox',async(req,res)=>{
  ...
  try{return res.json({ok:true,authority:'public.hipico_outbox',data:await listCanonicalOutbox(ownerId,{limit:limit(req.query.limit),status:String(req.query.status||'').trim()||undefined})});}
  catch(error:any){console.error('[hipico-outbox] list failed',{error:error?.message||String(error)});return res.status(503).json({ok:false,retryable:true,error:'canonical_outbox_unavailable'});}
});
...
function dispatchHttp(res:any,result:any){
  if(result.status==='accepted')return res.status(202).json({ok:true,retryable:false,data:result.row,providerMessageId:result.providerMessageId||null});
...
router.post('/test-message',async(req,res)=>{
  ...
  if(!(await canonicalOutboxReadiness()).ready)return res.status(503).json({ok:false,retryable:true,error:'canonical_outbox_unavailable'});
  try{
...
router.post('/approve/:id',async(req,res)=>{
  ...
  if(status==='failed'||status==='cancelled')return res.status(409).json({ok:false,retryable:false,error:'La salida está terminal y requiere revisión.'});
  ...
  catch(error:any){console.error('[hipico-outbox] approve failed',{id:item.id,error:error?.message||String(error),code:error?.code||null});return res.status(503).json({ok:false,retryable:false,error:'reconciliation_required'});}
});
...
router.post('/outbox/:id/reconcile',async(req,res)=>{
  ...
  const ownerId=configuredOutboxOwnerId();const actorRef=operatorActorRef();
  ...
  }catch(error:any){console.error('[hipico-outbox] reconcile failed',{id:parsedId.data,error:error?.message||String(error)});return res.status(503).json({ok:false,retryable:true,error:'No se pudo persistir la conciliacion.'});}
});
