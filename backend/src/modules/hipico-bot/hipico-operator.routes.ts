...
router.use((req,res,next)=>{
  ...
  if(!operatorTokenValid(req.header('x-hipico-operator-token')||undefined))return res.status(401).json({ok:false,error:'Token de operador Hipico invalido.'});
  next();
});

router.get('/status',async(_req,res)=>{
  const canonicalOutbox=await canonicalOutboxReadiness();
  return res.json({ok:true,data:{promotion:promotion(),dbReady:await HipicoBotStore.dbReady(),canonicalOutbox,cloudConfigured:cloudTransportConfiguration().configured,cloudOutboundPolicy:cloudOutboundPolicy(),webhookConfigured:webhookSecurityReady(),targetSupport:['individual'],groupAutomation:'bridge-required',groupQaMode:'shadow-only',raceProvider:raceProvider.status()}});
});
...
router.get('/race-provider/stages/:stageId',async(req,res)=>{
  try{return res.json({ok:true,data:await raceProvider.getLiveStage(String(req.params.stageId||''))});}
  catch(error:any){return res.status(hipicoProviderHttpStatus(error)).json({ok:false,...hipicoProviderPublicError(error)});}
});

router.get('/events',async(req,res)=>res.json({ok:true,data:await HipicoBotStore.events(limit(req.query.limit))}));
router.get('/outbox',async(req,res)=>{
  const ownerId=configuredOutboxOwnerId();if(!ownerId)return res.status(503).json({ok:false,error:'HIPICO_OWNER_NOT_CONFIGURED'});
  const status=String(req.query.status||'').trim();
  try{return res.json({ok:true,data:await listCanonicalOutbox(ownerId,{limit:limit(req.query.limit),status:status||undefined}),authority:'public.hipico_outbox'});}
  catch(error:any){console.error('[hipico-outbox] operator list failed',{error:error?.message||String(error)});return res.status(503).json({ok:false,retryable:true,error:'canonical_outbox_unavailable'});}
});
router.get('/shadow-projection',async(req,res)=>{
  const events=await HipicoBotStore.events(limit(req.query.limit||100)) as any[];
  return res.json({ok:true,data:buildShadowProjection(events.filter((event)=>String(event?.phoneNumberId||'').startsWith('group:')))});
});
...
router.post('/test-message',async(req,res)=>{
  const parsed=z.object({requestId:requestIdSchema,to:e164Schema,message:z.string().trim().min(1).max(4000),groupKey:groupKeySchema.default('operator-direct')}).safeParse(req.body);
  if(!parsed.success)return res.status(400).json({ok:false,error:'requestId, destino, groupKey o mensaje invalido.'});
  const ownerId=configuredOutboxOwnerId();if(!ownerId)return res.status(503).json({ok:false,retryable:true,error:'HIPICO_OWNER_NOT_CONFIGURED'});
  const preflight=outboundPreflight(parsed.data.to);if(!preflight.ok)return res.status(preflight.status).json({ok:false,retryable:preflight.retryable,error:preflight.error,reasons:preflight.reasons});
  const readiness=await canonicalOutboxReadiness();if(!readiness.ready)return res.status(503).json({ok:false,retryable:true,error:'canonical_outbox_unavailable'});
  try{
    const queued=await enqueueCanonicalOutbound({ownerId,groupKey:parsed.data.groupKey,destination:parsed.data.to,idempotencyKey:`operator-test:${parsed.data.requestId}`,replyType:'operator_test_message',payload:{text:parsed.data.message,source:'operator-test',approvalRequired:false}});
    const item=queued.row;const status=String(item.status||'');
    ...
    return dispatchHttp(res,await dispatchCanonicalOutbound({ownerId,id:String(item.id)}));
  }catch(error:any){
...
router.post('/approve/:id',async(req,res)=>{
  const parsedId=uuidSchema.safeParse(req.params.id);if(!parsedId.success)return res.status(400).json({ok:false,error:'ID de salida invalido.'});
  const ownerId=configuredOutboxOwnerId();if(!ownerId)return res.status(503).json({ok:false,retryable:true,error:'HIPICO_OWNER_NOT_CONFIGURED'});
  const item=await getCanonicalOutbox(ownerId,parsedId.data).catch(()=>null);if(!item)return res.status(404).json({ok:false,error:'Salida no encontrada.'});
...
router.post('/outbox/:id/reconcile',async(req,res)=>{
  ...
  const parsed=z.object({resolution:z.enum(['sent','failed']),reason:z.string().trim().min(5).max(500),providerMessageId:z.string().trim().min(1).max(320).nullable().optional()}).safeParse(req.body);
  if(!parsedId.success||!parsed.success)return res.status(400).json({ok:false,error:'Reconciliacion invalida.'});
  const ownerId=configuredOutboxOwnerId();const actorRef=operatorActorRef();if(!ownerId||!actorRef)return res.status(503).json({ok:false,error:'Identidad server-side de operador/propietario no configurada.'});
  ...
  try{
    const row=await reconcileCanonicalOutbound({ownerId,id:parsedId.data,resolution:parsed.data.resolution,actorRef,reason:parsed.data.reason,providerMessageId:parsed.data.providerMessageId});
    if(!row)return res.status(409).json({ok:false,retryable:false,error:'La salida no está en reconciliation_required.'});
    return res.json({ok:true,data:row});
  }catch(error:any){console.error('[hipico-outbox] reconciliation failed',{id:parsedId.data,error:error?.message||String(error)});return res.status(503).json({ok:false,retryable:true,error:'No se pudo persistir la conciliacion.'});}
});
