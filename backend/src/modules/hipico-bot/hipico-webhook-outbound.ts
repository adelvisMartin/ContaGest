...
export async function processIncomingCanonical(message:any,options:{requirePersistent?:boolean}={},deps:Deps=defaultDeps){
  ...
  const groupId=String(message.phoneNumberId||'').slice(0,180);
  const g=`meta-direct:${groupId||'unknown'}`;
  const trace=ownerId&&groupId
    ?buildObservationTrace({ownerId,groupKey:g,groupId,sourceRef:String(message.providerMessageId||'')})
    :null;
  ...
  await observe('PARSER','SUCCESS',String(result.reason||result.intent||'CLASSIFIED'),{
  ...
  await observe('OUTBOX',queued.inserted?'SUCCESS':'DUPLICATE',autoApproved?'CANONICAL_OUTBOX_AUTO_ELIGIBLE':'CANONICAL_OUTBOX_APPROVAL_REQUIRED',{
    mode,
