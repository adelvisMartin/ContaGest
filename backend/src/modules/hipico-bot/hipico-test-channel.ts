...
export class TestChannelAdapter {
  ...
  async inject(message: NormalizedChannelMessage) {
    ...
    if (!message || String(message.channel || '').trim() !== this.channel) {
      throw new Error('TEST_CHANNEL_IDENTITY_MISMATCH');
    }
    if (
      !String(message.groupId || '').trim()
      || !String(message.externalMessageId || '').trim()
