...
test('runner binds operator/session invariants and hashes evidence inputs',()=>{const script=fs.readFileSync('scripts/hipico-soak-v120.ts','utf8');assert.match(script,/operator-id/);assert.match(script,/session-fallback-safe/);assert.match(script,/samplesSha256/);assert.match(script,/drillEvidenceSha256/);assert.match(script,/schemaVersion:3/);});
