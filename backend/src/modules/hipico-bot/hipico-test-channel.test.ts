...
test('TestChannelAdapter fails closed while disconnected and preserves normalized identity', async () => {
...
test('TestChannelAdapter rejects wrong channel identity, empty destinations and empty payloads', async () => {
  const channel = new TestChannelAdapter('e2e');
  await channel.connect();
  await assert.rejects(channel.inject({ ...message(), channel: 'other' }), /TEST_CHANNEL_IDENTITY_MISMATCH/);
