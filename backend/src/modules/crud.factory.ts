...
function modelDelegate(model: CrudOptions['model']) {
  const delegate = (prisma as any)[model];
  if (!delegate) throw new Error(`Prisma model not found: ${String(model)}`);
  return delegate;
}
...
export function createCrudRouter(options: CrudOptions) {
  ...
  router.get('/', asyncHandler(async (req, res) => {
    ...
    const data = await modelDelegate(options.model).findMany({ where, orderBy: { createdAt: 'desc' }, take: Number(req.query.take || 100) });
    ok(res, data);
  }));
  ...
  router.get('/:id', asyncHandler(async (req, res) => {
    ...
    const data = await modelDelegate(options.model).findFirst({ where: options.tenantScoped === false ? where : { ...where, tenantId } });
  ...
  router.post('/', validateBody(options.schema), asyncHandler(async (req, res) => {
    ...
    const data = await modelDelegate(options.model).create({ data: payload });
  ...
  router.put('/:id', validateBody((options.schema as z.ZodObject<any>).partial()), asyncHandler(async (req, res) => {
    const ctx = (req as any).context;
    const before = await modelDelegate(options.model).findFirst({ where: options.tenantScoped === false ? { id: req.params.id } : { id: req.params.id, tenantId: ctx.tenantId } });
    if (!before) throw new HttpError(404, `${options.entity} no encontrado`);
    const data = await modelDelegate(options.model).update({ where: { id: req.params.id }, data: req.body });
  ...
  router.delete('/:id', asyncHandler(async (req, res) => {
    ...
    const before = await modelDelegate(options.model).findFirst({ where: options.tenantScoped === false ? { id: req.params.id } : { id: req.params.id, tenantId: ctx.tenantId } });
    if (!before) throw new HttpError(404, `${options.entity} no encontrado`);
    const data = await modelDelegate(options.model).delete({ where: { id: req.params.id } });
