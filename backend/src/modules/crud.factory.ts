...
export function createCrudRouter(options: CrudOptions) {
  ...
  router.get('/', asyncHandler(async (req, res) => {
    ...
    const data = await (await delegate()).findMany({ where, orderBy: { createdAt: 'desc' }, take: Number(req.query.take || 100) });
    ok(res, data);
  }));
  ...
  router.get('/:id', asyncHandler(async (req, res) => {
    ...
    const data = await (await delegate()).findFirst({ where: options.tenantScoped === false ? where : { ...where, tenantId } });
  ...
  router.post('/', validateBody(options.schema), asyncHandler(async (req, res) => {
    ...
    const data = await (await delegate()).create({ data: payload });
  ...
  router.put('/:id', validateBody((options.schema as z.ZodObject<any>).partial()), asyncHandler(async (req, res) => {
    const ctx = (req as any).context;
    const before = await (await delegate()).findFirst({ where: options.tenantScoped === false ? { id: req.params.id } : { id: req.params.id, tenantId: ctx.tenantId } });
    if (!before) throw new HttpError(404, `${options.entity} no encontrado`);
    const data = await (await delegate()).update({ where: { id: req.params.id }, data: req.body });
  ...
  router.delete('/:id', asyncHandler(async (req, res) => {
    ...
    const before = await (await delegate()).findFirst({ where: options.tenantScoped === false ? { id: req.params.id } : { id: req.params.id, tenantId: ctx.tenantId } });
    if (!before) throw new HttpError(404, `${options.entity} no encontrado`);
    const data = await (await delegate()).delete({ where: { id: req.params.id } });
