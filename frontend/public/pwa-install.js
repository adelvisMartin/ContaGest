(() => {
  ...
  const appIcon = () => document.querySelector('link[rel="icon"]')?.href || document.querySelector('link[rel="apple-touch-icon"]')?.href || '';
  ...
  function mountInstallBanner() {
    ...
    host.innerHTML = `
      ...
      <div class="cg-pwa-row"><img class="cg-pwa-icon" src="${appIcon()}" alt=""><div class="cg-pwa-copy"><p class="cg-pwa-title">Instalar ContaGest</p><p class="cg-pwa-text">Úsala como una app del teléfono, en pantalla completa y con acceso desde el inicio.</p></div></div>
    ...
    host.querySelector('[data-cg-install]')?.addEventListener('click', async () => {
      ...
      if (isIos) {
        const existing = host.querySelector('[data-cg-ios-help]');
        if (existing) return;
        const message = document.createElement('div');
        message.dataset.cgIosHelp = 'true';
