...
export function validateBackupObject(parsed, { maxLocalSchemaVersion = LOCAL_SCHEMA_VERSION } = {}) {
  ...
  if (!parsed._backup) return { legacy: true, workspace: parsed, manifest: null, warnings: ['LEGACY_UNHASHED_BACKUP'] };
  ...
  const actual = sha256Sync(stable(stripBackupSecrets(workspace)));
  ...
  return { legacy: false, workspace, manifest, warnings: [] };
}
