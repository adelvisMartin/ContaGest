...
export function compareRuntimeVersion(buildInfo, expectedVersion = APP_VERSION) {
  ...
  return {
    ok: Boolean(observed && expected && observed === expected),
    expected,
...
export async function checkRuntimeVersion(fetchImpl = globalThis.fetch) {
  ...
  try {
    ...
    // Network absence is handled by the offline trust banner. Do not turn a normal
    // offline session into a false version mismatch when metadata cannot be fetched.
