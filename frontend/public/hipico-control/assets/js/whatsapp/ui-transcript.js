...
function documentInfo(filename){
  ...
  return {
    ...
    code:dateMatch?base.slice(0,-8):base,
    embeddedDate:dateMatch?`${dateMatch[3]}-${dateMatch[2]}-${dateMatch[1]}`:''
  };
}
...
export function parseWhatsAppUiTranscript(input){
  ...
  const ensurePending=()=>{
    if(!pending)pending={...lastIdentity,dayLabel,forwarded:false,body:[],media:null};
    return pending;
  };
  const pushMessage=(time='',forcedKind='')=>{
    ...
    messages.push({
      id:`MSG-${messages.length+1}-UI`,
      time,
      date:'',
      dateLabel:pending.dayLabel||dayLabel,
      timestamp:null,
      sender:pending.sender||'Sin identificar',
      senderKey:senderKey(pending.sender,pending.phone),
      phone:pending.phone||'',
      text,
      sourceFormat:'whatsapp-ui-copy',
      forwarded:Boolean(pending.forwarded),
      systemKind:kind==='text'?'':kind,
      media:pending.media||null
    });
    pending=null;
  };
  const pushNoise=(text,reason)=>{
    messages.push({
      id:`MSG-${messages.length+1}-UI-NOISE`,
      time:TIME_RE.test(text)?text:'',
      date:'',
      dateLabel:dayLabel,
      timestamp:null,
      sender:lastIdentity.sender||'Sin identificar',
      senderKey:senderKey(lastIdentity.sender,lastIdentity.phone),
      phone:lastIdentity.phone||'',
      text,
      sourceFormat:'whatsapp-ui-copy',
      forwarded:false,
      systemKind:'noise',
  ...
  for(let index=0;index<lines.length;index+=1){
    ...
    if(next&&PHONE_RE.test(next)&&!TIME_RE.test(line)){
      pushMessage();
      lastIdentity={sender:line,phone:next};
      index+=1;
      continue;
    }
    if(FORWARDED_RE.test(line)){
      ensurePending().forwarded=true;
      continue;
    }
    if(TIME_RE.test(line)){
      if(pending&&(pending.body.length||pending.media))pushMessage(line);
      else pushNoise(line,'orphan_time');
      continue;
    }
    if(STICKER_RE.test(line)){
      const target=ensurePending();
      target.body=[line];
      target.media={kind:'sticker'};
      continue;
    }
    ...
    if(metadata){
      const target=ensurePending();
      if(target.media){
        target.media.pages=Number(metadata[1]);
        target.media.mediaType=metadata[2].toUpperCase();
        target.media.size=Number(metadata[3].replace(',','.'));
        target.media.sizeUnit=metadata[4].toUpperCase();
      }else{
