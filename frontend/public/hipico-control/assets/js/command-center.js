...
export function renderCommandCenter(state) {
  ...
  return `${staleNotice}<section class="page-head section-gap" data-command-center><div><h2>Command Center</h2><p>Estado observable del backend canónico para ${escapeHtml(data.scope?.groupKey || 'grupo activo')} · lectura no cacheada.</p></div><div class="page-actions"><span class="badge ${data.system?.ok && !critical ? 'badge--success' : 'badge--warning'}">${data.system?.ok && !critical ? 'OPERATIVO' : 'ATENCIÓN'}</span><button class="button" data-action="refresh-command-center">Actualizar</button></div></section><div class="grid grid--kpi metrics-grid">${componentCard('Backend', components.backend)}${componentCard('PostgreSQL', components.database)}${componentCard('Bridge', bridge)}${componentCard('Canal', channel)}${componentCard('Providers', providerComponent)}${componentCard('Agente', agent)}</div><div class="grid grid--two section-gap"><section class="card"><div class="card__head"><div><h3>Operación canónica</h3><small>${data.operation?.state === 'unavailable' ? 'Lectura no disponible' : meeting ? escapeHtml(meeting.name) : 'Sin meeting activo'}</small></div><span class="badge ${badgeClass(data.operation?.state)}">${data.operation?.raceCount == null ? 'N/D' : `${data.operation.raceCount} carreras`}</span></div><div class="card__body"><div class="control-stack">${raceLine('Carrera actual', data.operation?.currentRace, data.operation?.state)}${raceLine('Próxima carrera', data.operation?.nextRace, data.operation?.state)}<div><span>Cola pendiente</span><strong>${queueUnavailable ? 'No disponible' : countValue(data.queue?.pending)}</strong><small>${queueUnavailable ? 'Lectura fallida' : `${countValue(data.queue?.failed)} fallidas`}</small></div><div><span>Conflictos</span><strong>${conflictUnavailable ? 'No disponible' : countValue(data.conflicts?.total)}</strong><small>${conflictUnavailable ? 'Lectura fallida' : 'reconciliación + transiciones + agente'}</small></div></div></div></section><section class="card"><div class="card__head"><div><h3>Alertas operativas</h3><small>Evidencia observable; no modifica jugadas ni saldos.</small></div><span class="badge ${critical ? 'badge--danger' : alerts.length ? 'badge--warning' : 'badge--success'}">${alerts.length}</span></div><div class="card__body"><div class="list" aria-live="polite">${alerts.length ? alerts.map((item) => `<div class="audit-row"><strong>${escapeHtml(item.message)}</strong><span>${escapeHtml(item.code)}</span><small>${escapeHtml(item.severity.toUpperCase())}</small></div>`).join('') : '<div class="check-list"><div class="is-good">✓ Sin alertas observables en esta lectura.</div></div>'}</div></div></section></div><section class="card section-gap"><div class="card__head"><div><h3>Documentos recientes</h3><small>Clasificación, autoridad y estado del motor PDF.</small></div><span class="badge ${documentUnavailable ? 'badge--danger' : ''}">${documentUnavailable ? 'N/D' : documents.length}</span></div><div class="card__body"><div class="responsive-records">${documentUnavailable ? '<div class="empty"><strong>No disponible</strong><span>No se pudo leer la evidencia documental. Revisa las alertas operativas.</span></div>' : documents.length ? documents.slice(0, 8).map((document) => `<article><strong>${escapeHtml(document.filename || 'Documento')}</strong><span>${escapeHtml(document.classification || 'UNKNOWN')} · ${escapeHtml(document.status || 'unknown')}</span><small>${escapeHtml(document.authority || 'unknown')} · ${escapeHtml(formatDate(document.updatedAt || document.createdAt))}</small></article>`).join('') : '<div class="empty"><strong>Sin documentos</strong><span>No hay evidencia documental persistida para este grupo.</span></div>'}</div></div></section>`;
}
