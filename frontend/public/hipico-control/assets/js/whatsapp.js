...
function findTrack(text, catalog = []) {
  ...
  if (normalized === "INDIANAPOLIS" || normalized === "INDIANAPOLIS") return "Horseshoe Indianapolis";
...
function extractHorse(textBeforeAmount, playMatches) {
...
function detectPairOnly(textBeforeAmount) {
...
export function parseWhatsAppChat(input, options = {}) {
  ...
  for (const line of lines) {
    ...
    if (header) {
      ...
      current = {
        ...
        time, date, timestamp: parseDateTime(date, time), sender,
        senderKey: phone || compact(sender), phone, text: text || ""
      };
      messages.push(current);
    } else if (current) {
      current.text += `${current.text ? "\n" : ""}${line}`;
    } else if (line.trim()) {
      current = { id: `MSG-${messages.length + 1}-SIN-REMITENTE`, time: "", date: "", timestamp: null, sender: "Sin identificar", senderKey: "SIN-IDENTIFICAR", phone: "", text: line.trim() };
      messages.push(current);
    }
  }
  ...
  for (const offer of offers) {
    ...
    } else seen.set(key, offer);
  }
  const activeOffers = offers.filter((offer) => !offer.duplicate);
  const replies = typed.filter((message) => message.type === "reply" && message.reply).map((message) => ({ ...message.reply, segmentId: message.segmentId }));
  ...
  return {
    ...
    segments: Math.max(1, segmentId - (typed.at(-1)?.type === "closure" ? 1 : 0)),
    stats: {
...
export function participantMatchesSender(participant, sender, phone = "") {
  ...
  const values = [participant?.code, participant?.name, participant?.alias].filter(Boolean).map(compact);
...
export function generateClosureText(companyName = "CLUB HIPICO TRIPLE CROWN", scope = "AMERICANAS") {
  return [`🇺🇸🏇🏻 ${scope.toUpperCase()} 🇺🇸🏇🏻`, "🔓🇨 🇪 🇷 🇷 🇦 🇩 🇴🔒", "📵🇨 🇪 🇷 🇷 🇦 🇩 🇴 📵", "      🏇🏇🏇🏇🏇🏇", "  ⚠️ 𝑵𝑶 𝑴𝑨𝑺 𝑱𝑼𝑮𝑨𝑫𝑨𝑺 ⚠️", "📵📵🛑🛑 CERRADO CERRADO 🛑🛑📵📵", "🔒 CARRERA CERRADA 🔒", "🛑🚫 NO HAY MÁS JUGADAS 🚫🛑", `🏇 ${companyName}`, "NOS REGIMOS POR EL CHAT📲 Y NUESTROS REGLAMENTOS📝", "👀 TODO TERCIO VA JUGANDO CON SU DISPONIBLE EN EL PLANO POZO. LA CASA NO PAGA EXCEDENTES SI SE PASAN DE SU MONTO, SALVO VALIDACIÓN DE UN ADMINISTRADOR."].join("\n");
}
