...
const normalizeRoleDefinition = (role={}) => {
  const shared=roleAccessProfile(role.id);
  const modules=canonicalModules(shared?.modules||role.modules);
  const capabilityPermissions=(shared?.capabilities||role.permissions||[]).map(String).filter((permission)=>!routePermissions.has(permission));
  return {
    ...role,
    modules,
...
const ROLE_METADATA = [
  { id:'role-admin', name:'Administrador', tone:'danger', description:'Control total de empresa, usuarios, permisos, seguridad, integraciones, reportes y módulos.', scope:'Acceso completo. Puede administrar usuarios, límites, módulos, bitácora, monitoreo, integraciones y configuración empresarial.' },
  { id:'role-gerente', name:'Gerente / Dueño', tone:'brand', description:'Visión ejecutiva, ventas, inventario, caja, analítica y reportes.', scope:'Control operativo y ejecutivo. Puede revisar KPIs, ventas, compras, inventario, bancos, reportes y auditoría sin administrar secretos técnicos.' },
  { id:'role-contador', name:'Contador', tone:'brand', description:'Contabilidad, fiscal, compras, ventas, reportes y auditoría documental.', scope:'Registra y revisa asientos, libros, impuestos, retenciones, compras, ventas y reportes. No administra usuarios ni permisos globales.' },
  { id:'role-tesoreria', name:'Tesorería / Finanzas', tone:'slate', description:'Bancos, caja, conciliación, cobranza, pagos y reportes financieros.', scope:'Gestiona bancos, movimientos, cuentas por cobrar/pagar, reportes y auditoría financiera.' },
  { id:'role-vendedor', name:'Vendedor / Caja', tone:'success', description:'CRM, cotizaciones, ventas, pedidos y POS.', scope:'Crea clientes, cotizaciones, ventas, pedidos y operaciones de caja. Puede consultar seguimiento y soporte.' },
  { id:'role-clinica', name:'Clínica / Consultorio', tone:'brand', description:'Pacientes, agenda, historias clínicas, facturación y operación del consultorio.', scope:'Perfil compacto para profesionales y consultorios pequeños. RRHH y nómina se habilitan aparte cuando exista personal adicional.' },
  { id:'role-veterinaria', name:'Clínica veterinaria', tone:'success', description:'Mascotas, tutores, historias, vacunas, inventario, compras y facturación.', scope:'Perfil compacto para veterinarios y PYMES. RRHH y nómina se habilitan aparte cuando exista personal adicional.' },
  { id:'role-psicologia', name:'Psicología / Consultorio', tone:'brand', description:'Pacientes, citas, agenda semanal, confirmaciones, seguimiento y cobranza del consultorio.', scope:'Perfil para psicólogos particulares y centros pequeños. Prioriza agenda, pacientes y comunicaciones sin exponer módulos de RRHH por defecto.' },
  { id:'role-odontologia', name:'Odontología / Consultorio dental', tone:'brand', description:'Pacientes, odontograma, tratamientos, citas, presupuestos, seguimiento y cobranza.', scope:'Perfil para odontólogos y clínicas dentales pequeñas. Prioriza historia odontológica, procedimientos, agenda y comunicación con el paciente.' },
  { id:'role-gimnasio', name:'Gimnasio / Fitness', tone:'success', description:'Socios, membresías, asistencia, rutinas, nutrición y cobranza.', scope:'Gestiona la operación del gimnasio, seguimiento de socios, planes, rutinas y reportes.' },
  { id:'role-nutricion', name:'Nutrición / Consulta', tone:'success', description:'Pacientes, planes alimentarios, composición, adherencia y seguimiento.', scope:'Perfil independiente para profesionales de nutrición. Usa la capacidad nutricional del dominio Fitness sin habilitar por defecto la operación del gimnasio.' },
  { id:'role-inventario', name:'Inventario / Almacén', tone:'warning', description:'Productos, stock, kardex, escaneo, alertas y movimientos.', scope:'Controla productos, existencias, kardex, escaneo y reportes de stock.' },
  { id:'role-compras', name:'Compras / Proveedores', tone:'warning', description:'Proveedores, órdenes de compra, costos e inventario entrante.', scope:'Gestiona proveedores, compras y seguimiento de inventario recibido.' },
  { id:'role-rrhh', name:'RRHH', tone:'slate', description:'Nómina, empleados, incidencias y reportes humanos.', scope:'Trabaja nómina, RRHH y reportes del área.' },
  { id:'role-auditor', name:'Auditor interno', tone:'danger', description:'Lectura, auditoría, bitácora, controles y reportes de cumplimiento.', scope:'Perfil de revisión para reportes, auditoría y controles, sin editar operaciones transaccionales.' },
  { id:'role-soporte', name:'Soporte técnico interno', tone:'accent', description:'Monitoreo, bitácora, fallas, integraciones y asistencia.', scope:'Revisa fallas, monitoreo, integraciones, soporte y estado del sistema. No cambia roles sin autorización administrativa.' },
  { id:'role-demo', name:'Acceso temporal', tone:'accent', description:'Acceso comercial con vencimiento y módulos seleccionados.', scope:'Acceso temporal para prospectos. El administrador define vigencia, máximo de módulos y alcance.' },
  { id:'role-lectura', name:'Solo lectura', tone:'slate', description:'Consulta ejecutiva sin modificar datos.', scope:'Puede revisar dashboard, reportes y ayuda sin editar registros ni configuraciones sensibles.' }
];view'],
    modules:['dashboard','reportes','analytics','ayuda','soporte']
  }
 ];

const ROLE_DEFINITIONS = ROLE_METADATA.map((role)=>normalizeRoleDefinition(role));
