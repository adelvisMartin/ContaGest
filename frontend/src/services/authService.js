...
function normalizeSession(payload){const tenantId=payload?.tenantId||payload?.tenant?.id;if(!payload?.token||!tenantId)throw new Error('El backend no devolvió una sesión firmada completa.');return AuthSession.set({token:payload.token,tenantId,tenant:payload.tenant||null,user:payload.user||null,license:payload.license||null,expiresAt:payload.expiresAt||Date.now()+1000*60*60*8,mode:'api'});}
export const AuthService={
  ...
  async login({email,password,tenantRif='00000000',captchaToken='',captchaAnswer='',licenseKey='',deviceId='',deviceLabel='',mode='api'}){if(mode==='demo'){if(!demoModeEnabled())throw new Error('El modo demo local está deshabilitado en esta compilación.');if(!email||!password)throw new Error('Ingresa email y contraseña.');return AuthSession.set({token:`demo.${Date.now()}`,user:{...DEMO_USER,email},tenantId:'demo-tenant',tenant:{id:'demo-tenant',name:'Demo local',rif:'00000000',plan:'development'},expiresAt:Date.now()+1000*60*60*8,mode:'demo'});}const payload=await BackendApi.request('/auth/login',{method:'POST',noAuth:true,body:{email,password,tenantRif,captchaToken,captchaAnswer,...(licenseKey?{licenseKey,deviceId:deviceId||LicenseService.deviceId(),deviceLabel:deviceLabel||LicenseService.deviceLabel()}:{})}});if(payload?.mfaRequired)return payload;return normalizeSession(payload);},
