...
export function createDefaultState() {
  return {
    version: 'v10.4.0-enterprise-rbac-monitoring-hardening',
    route: 'dashboard',
    settings: {
      theme: 'light',
      businessMode: 'admin',
      ...
      sheetsUrl: '',
      companyName: 'ContaGest-VE',
      companyTradeName: 'ContaGest Demo',
      companyRif: 'J123456789',
      companyAddress: 'Caracas, Venezuela · Sucursal Central',
      companyPhone: '+58 412-0000000',
      companyEmail: 'administracion@contagest.ve',
      companyWebsite: 'www.contagest.ve',
      companySlogan: 'Documento comercial tributario · Vista previa de gestión',
      companyLogoDataUrl: '',
      documentFooter: 'Documento referencial sujeto a validación normativa, correlativo fiscal, revisión contable y auditoría interna.',
      documentLegalNote: 'Este comprobante debe validarse contra los libros fiscales y las normas tributarias vigentes antes de su emisión definitiva.',
      brandPrimary: '#00236f',
      brandSurface: '#f7f9fb'
    },
    ...
    quote: {
      date: today(),
      numeroFactura: `FAC-${new Date().getFullYear()}-001`,
      orden: `ORD-${new Date().getFullYear()}-001`,
      clientId: '',
      currency: 'USD',
      manualAmount: 0,
      observation: '',
      ...
      taxes: {
        ...
        igtf: { active: false, rate: 3 },
        islr: { active: false, rate: 2 },
        custom: { active: false, rate: 0, label: 'Otro tributo' },
        retIva: { active: false, rate: 75 },
        retIslr: { active: false, rate: 2 }
      }
    },
    calculation: {},
    clients: [
      { id: uid('cli'), name: 'Empresa Demo C.A.', rif: 'J123456789', email: 'compras@clientedemo.com', phone: '+58 412-0000000', type: 'Contribuyente ordinario', address: 'Av. Principal, Caracas' },
    ...
    inventoryMovements: [],
    ledger: {
      accounts: ['Caja', 'Bancos', 'Cuentas por cobrar', 'Inventario', 'IVA crédito fiscal', 'IVA débito fiscal', 'Ventas', 'Compras', 'Gastos administrativos', 'Retenciones por pagar'],
      entries: []
    },
    accounting: {
      adjustments: [],
      periods: [],
      closePolicy: { requireBalancedTrialBalance: true, requireLockedTaxes: true, requireBankReconciliation: false }
    },
    banking: {
      accounts: [
        { id: uid('bank'), bank: 'Banesco', account: '0102-****-0001', currency: 'VES', balance: 0 },
        { id: uid('bank'), bank: 'Mercantil', account: '0105-****-0002', currency: 'USD', balance: 0 }
      ],
      movements: []
    },
    payroll: { records: [] },
    suppliers: [
      { id: uid('sup'), name: 'Proveedor Demo CA', rif: 'J111111111', email: 'facturacion@proveedor.com', phone: '+58 414-1111111', category: 'Insumos' }
    ],
    purchases: [],
    inventoryScans: [],
    ...
    profile: { name: 'Admin User', email: 'admin@empresa.com', role: 'Administrador', branch: 'Sucursal Central', plan: 'Enterprise', avatarDataUrl: '' },
    admin: { activeUsers: 1248, apiRequests24h: 84000, storagePct: 64, licensesUsed: 45, licensesTotal: 50, billingCutoff: '2024-11-15' },
    ...
    auditLog: [],
    failureLog: FailureLogService.defaultState(),
    moduleRuntime: {},
    security: { rls: true, rbac: true, auditLog: true, rateLimit: true, corsStrict: true, csrfReady: true },
    analytics: { events: [], visits: {}, sessions: [], lastEventAt: null },
    qr: { defaultErrorCorrection: 'M', labelSize: '70x40mm', verifyBaseUrl: '/verify' },
    fastFoodMenu: [
    ...
    support: { whatsapp: '+584120000000', email: 'soporte@contagest.ve', sla: 'L-V 8:00am a 6:00pm' },
    delivery: { origin: 'Sucursal Central, Caracas, Venezuela', provider: 'google_maps', fallbackProvider: 'mapbox' }
  };
}
