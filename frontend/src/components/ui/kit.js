...
export const icon = (name = 'fa-circle-dot', className = '') => {
  ...
  return `<i class="fa-solid ${safe(fa)} ${safe(className)}"></i>`;
};
...
const normalizeVariant = (variant = 'primary') => ({ accent:'primary', outline:'secondary', subtle:'secondary', tertiary:'secondary' }[variant] || variant || 'primary');
...
export function Button(args = {}) {
  ...
  return `<button ${type} ${id} class="cgx-btn cgx-btn-${safe(variant)} btn btn-${safe(variant)} ds-btn ds-btn-${safe(variant)} cgv-btn cgv-btn-${safe(variant)} ${className}" ${route} ${attrs} data-mui-button-fallback data-cgx-kit="button">${iconName ? icon(iconName) : ''}<span ${i18n}>${safe(label)}</span></button>`;
}
...
export function PageHeader(args = {}) {
  const eyebrow = args.eyebrow ?? args.eyebrowKey ?? 'ContaGest ERP';
  ...
  return `<header class="cgx-page-header" data-cgx-kit="page-header"><div class="cgx-page-copy"><p class="cgx-eyebrow" ${args.eyebrowKey ? `data-i18n="${safe(args.eyebrowKey)}"` : ''}>${safe(eyebrow)}</p><h1 ${args.titleKey ? `data-i18n="${safe(args.titleKey)}"` : ''}>${safe(title)}</h1>${description ? `<p ${args.descKey ? `data-i18n="${safe(args.descKey)}"` : ''}>${safe(description)}</p>` : ''}${meta.length ? `<div class="cgx-meta-row">${meta.map((item) => `<span>${safe(item)}</span>`).join('')}</div>` : ''}</div>${actions ? `<div class="cgx-page-actions">${actions}</div>` : ''}</header>`;
}

export function MetricCard({ label = '', value = '-', hint = '', iconName = 'fa-chart-simple', tone = 'brand', trend = '' } = {}) {
  return `<article class="cgx-metric cgx-metric-${safe(toneMap(tone))}" data-cgx-kit="metric"><div class="cgx-metric-icon">${icon(iconName)}</div><div><p>${safe(label)}</p><strong>${safe(value)}</strong>${hint ? `<small>${safe(hint)}</small>` : ''}</div>${trend ? `<span class="cgx-trend">${safe(trend)}</span>` : ''}</article>`;
}
...
export const KpiCard = ({ labelKey = '', valueId = '', subId = '', icon: iconName = 'fa-chart-simple', value = '-', sub = '', className = '' } = {}) =>
  `<article class="cgx-metric kpi ${safe(className)}" data-cgx-kit="kpi"><div class="cgx-metric-icon">${icon(iconName)}</div><div><p data-i18n="${safe(labelKey)}">${safe(labelKey)}</p><strong id="${safe(valueId)}">${safe(value)}</strong>${subId ? `<small id="${safe(subId)}">${safe(sub)}</small>` : ''}</div></article>`;
...
export function Field({ labelKey = '', name = '', id = '', type = 'text', required = false, placeholderKey = '', placeholder = '', value = '', attrs = '', className = '', inputClass = '' } = {}) {
  return `<div class="cgx-field ${safe(className)}" data-cgx-kit="field"><label class="label cgx-label" ${labelKey ? `data-i18n="${safe(labelKey)}"` : ''}>${safe(labelKey)}</label><input ${id ? `id="${safe(id)}"` : ''} name="${safe(name)}" type="${safe(type)}" ${required ? 'required' : ''} value="${safe(value)}" ${placeholderKey ? `data-i18n-placeholder="${safe(placeholderKey)}"` : ''} ${placeholder ? `placeholder="${safe(placeholder)}"` : ''} class="input cgx-field-normalized ${safe(inputClass)}" ${rawAttrs(attrs)} /></div>`;
}
export function Textarea({ labelKey = '', name = '', placeholderKey = '', placeholder = '', value = '', required = false, className = '', attrs = '' } = {}) {
  return `<div class="cgx-field ${safe(className)}" data-cgx-kit="textarea"><label class="label cgx-label" ${labelKey ? `data-i18n="${safe(labelKey)}"` : ''}>${safe(labelKey)}</label><textarea name="${safe(name)}" ${required ? 'required' : ''} ${placeholderKey ? `data-i18n-placeholder="${safe(placeholderKey)}"` : ''} ${placeholder ? `placeholder="${safe(placeholder)}"` : ''} class="textarea cgx-field-normalized" ${rawAttrs(attrs)}>${safe(value)}</textarea></div>`;
}
export function Select({ labelKey = '', name = '', options = [], value = '', className = '', attrs = '' } = {}) {
  return `<div class="cgx-field ${safe(className)}" data-cgx-kit="select"><label class="label cgx-label" ${labelKey ? `data-i18n="${safe(labelKey)}"` : ''}>${safe(labelKey)}</label><select name="${safe(name)}" class="select cgx-field-normalized mui-fallback-select" ${rawAttrs(attrs)}>${options.map((option) => `<option value="${safe(option.value)}" ${String(option.value) === String(value) ? 'selected' : ''}>${safe(option.label ?? option.value)}</option>`).join('')}</select></div>`;
}

export function EmptyState({ title = 'Sin datos', description = '', iconName = '', icon: iconArg = '' } = {}) {
  return `<div class="cgx-empty" data-cgx-kit="empty"><span>${icon(iconName || iconArg || 'fa-inbox')}</span><strong>${safe(title)}</strong>${description ? `<p>${safe(description)}</p>` : ''}</div>`;
}
export function Toolbar({ title = '', searchPlaceholder = 'Buscar...', actions = '' } = {}) {
  return `<div class="cgx-toolbar" data-cgx-kit="toolbar"><strong>${safe(title)}</strong><label><i class="fa-solid fa-magnifying-glass"></i><input type="search" placeholder="${safe(searchPlaceholder)}" /></label><div>${actions}</div></div>`;
}
...
export const TopIconButton = ({ icon: iconName, label, route = '', action = '', endpoint = '' } = {}) =>
  `<button type="button" class="hf-icon-button icon-action-button cgx-btn-icon" aria-label="${safe(label)}" ${route ? `data-route="${safe(route)}"` : ''} ${action ? `data-backend-action="${safe(action)}"` : ''} ${endpoint ? `data-endpoint="${safe(endpoint)}"` : ''}>${icon(iconName)}</button>`;
...
export const EnterpriseShellHeader = ({ title, subtitle = '', actions = '' } = {}) => PageHeader({ eyebrow: 'ContaGest ERP', title, description: subtitle, actions });
...
export const DS = {
  ...
  PageHeader({ title = '', subtitle = '', actions = '' } = {}) { return PageHeader({ eyebrow: 'ContaGest ERP', title, description: subtitle, actions }); },
  ...
  Form({ id = '', fields = [], submitLabel = 'Guardar' } = {}) {
    return `<form id="${safe(id)}" class="cgx-form-normalized ds-form" data-cgx-kit="form"><div class="cg-record-fields">${fields.map((field) => Field({ labelKey: field.label, name: field.name, type: field.type || 'text', placeholder: field.placeholder || '', value: field.value || '', required: Boolean(field.required) })).join('')}</div>${Button({ text: submitLabel, icon: 'fa-floppy-disk', type: 'submit' })}</form>`;
  },
  ResourcePage({ title = '', subtitle = '', actions = '', kpis = [], columns = [], rows = [] } = {}) {
    return `<section class="cgx-page ds-resource-page" data-cgx-kit="resource-page">${DS.PageHeader({ title, subtitle, actions })}${MetricGrid(kpis.map((k) => ({ ...k, hint: k.sub, iconName: k.iconName })))}${DataTable({ columns, rows })}</section>`;
  }
};

export const UI = {
  icon, Button, Badge, PageHeader, MetricCard, MetricGrid, Section, DataTable, EmptyState, Toolbar, Timeline,
  KpiCard, StatCard, Field, Select, Textarea, Table, FiscalTable, EnterpriseButton, EnterpriseKpi, EnterpriseShellHeader, TopIconButton, MaterialIcon, Icon, DS
};
