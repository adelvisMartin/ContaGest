...
export function FitnessNutritionQuickTool({members,Toast,onDataChanged}){
  ...
  return <Stack gap={1.25}>
    ...
    <Paper className="cg-fooddata-tool" variant="outlined" sx={{p:1.5}}>
      <Typography variant="h6">Verificar alimento · USDA FoodData Central</Typography><Typography variant="caption" color="text.secondary">La clave API permanece sólo en servidor. Foundation y SR Legacy se pueden agregar con snapshot de 100 g; otros tipos requieren base manual explícita.</Typography>
      ...
      <Stack gap={.7} mt={1.2}>{foods.length?foods.map((food)=><Paper key={food.fdcId||food.description} variant="outlined" sx={{p:1}}><Stack direction={{xs:'column',sm:'row'}} justifyContent="space-between" gap={1}><Box><Typography variant="body2" fontWeight={700}>{food.description}</Typography><Typography variant="caption" color="text.secondary">{food.brandOwner||food.dataType||'USDA'}</Typography><Typography variant="body2" mt={.4}>{Math.round(Number(food.nutrients?.calories||0))} kcal · P {Number(food.nutrients?.proteinG||0).toFixed(1)}g · C {Number(food.nutrients?.carbsG||0).toFixed(1)}g · G {Number(food.nutrients?.fatG||0).toFixed(1)}g</Typography><Typography variant="caption" color="text.secondary">Fibra {Number(food.nutrients?.fiberG||0).toFixed(1)}g · Na {Math.round(Number(food.nutrients?.sodiumMg||0))}mg · K {Math.round(Number(food.nutrients?.potassiumMg||0))}mg</Typography></Box><CgButton size="small" variant="outlined" onClick={()=>void addUsdaIngredient(food)} disabled={foodLoading||!/^(Foundation|SR Legacy)$/i.test(String(food.dataType||''))}>Agregar ingrediente</CgButton></Stack></Paper>):<CgEmptyState title="Sin resultados USDA" description="Busca un alimento real para revisar energía, proteína y micronutrientes."/>}</Stack>
    </Paper>
  </Stack>;
}
