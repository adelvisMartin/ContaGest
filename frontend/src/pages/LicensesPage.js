...
function commercialPanel(state){
  ...
  const loading=!c.loaded?'<div class="panel-soft p-4" role="status"><i class="fa-solid fa-spinner fa-spin"></i> Cargando datos comerciales…</div>':'';
  return `<section class="cgx-section" aria-labelledby="commercialTitle">
    ...
    <div class="cgx-section-body grid gap-4">${error}${loading}
      ...
      <section><header class="cg-license-form-head"><div><h3>Actividad comercial auditable</h3><p>Operaciones `commercial.*` del contexto administrativo activo; no muestra secretos ni payloads completos.</p></div></header><div class="pl-table-wrap"><table class="pl-table"><thead><tr><th>Acción</th><th>Entidad</th><th>Actor</th><th>Fecha</th></tr></thead><tbody>${activityRows(activity)}</tbody></table></div></section>
    </div>
  </section>`;
}
  ...
  mount(_state, { Store, Toast, Modal }) {
    ...
    const loadCommercial=async({silent=false}={})=>{
      ...
      try{
        ...
        Store.update(draft=>{draft.commercial={...(draft.commercial||{}),summary,customers,subscriptions,agents,renewals,plans,payments,commissions,activity,loaded:true,error:null};});
        if(!silent)Toast.show('Backoffice comercial actualizado.','success');
    ...
    document.getElementById('commercialFilterForm')?.addEventListener('submit',(event)=>{event.preventDefault();const f=event.currentTarget;Store.update(draft=>{draft.commercial={...(draft.commercial||{}),filters:{q:f.q.value,planCode:f.planCode.value,salesAgentId:f.salesAgentId.value,vertical:f.vertical.value,status:f.status.value}};});});
    ...
    subscriptionForm?.planCode?.addEventListener('change',()=>{const template=Store.get().commercial?.plans?.plans?.[subscriptionForm.planCode.value];if(!template)return;subscriptionForm.maxTenants.value=String(template.maxTenants||1);subscriptionForm.maxUsers.value=String(template.maxUsers||3);});
    ...
    document.getElementById('licenseForm')?.addEventListener('submit', async (event) => {
      ...
      try {const result = await LicenseService.generate({fullName:form.fullName.value,userEmail:form.userEmail.value,businessSector:form.businessSector.value,commercialUse:form.commercialUse.value,plan:form.plan.value,days:Number(form.days.value),maxUsers:Number(form.maxUsers.value),maxDevices:Number(form.maxDevices.value),modules,...(form.subscriptionId?.value?{subscriptionId:form.subscriptionId.value}:{})});Store.update((draft) => {draft.licenseCredentials=result.credentials;draft.licenses=[result,...(draft.licenses||[]).filter((item)=>item.id!==result.id)];});Toast.show('Licencia, usuario y contraseña temporal creados.','success');}
      catch (error) {Toast.show(`No se pudo generar la licencia: ${error.message}`, 'error');} finally {button?.removeAttribute('disabled');}
    });

    document.getElementById('commercialCustomerForm')?.addEventListener('submit',async(event)=>{event.preventDefault();const f=event.currentTarget;try{await CommercialService.createCustomer({legalName:f.legalName.value,rif:f.rif.value||undefined,contactName:f.contactName.value||undefined,email:f.email.value||undefined});Toast.show('Cliente comercial creado.','success');await loadCommercial({silent:true});}catch(error){Toast.show(error.message,'error');}});
    document.getElementById('commercialAgentForm')?.addEventListener('submit',async(event)=>{event.preventDefault();const f=event.currentTarget;try{await CommercialService.createAgent({name:f.name.value,email:f.email.value||undefined,phone:f.phone.value||undefined,commissionRate:Number(f.commissionRate.value)});Toast.show('Vendedor registrado.','success');await loadCommercial({silent:true});}catch(error){Toast.show(error.message,'error');}});
    subscriptionForm?.addEventListener('submit',async(event)=>{event.preventDefault();const f=event.currentTarget;if(!f.customerAccountId.value)return Toast.show('Selecciona un cliente.','warning');try{const template=Store.get().commercial?.plans?.plans?.[f.planCode.value]||{};await CommercialService.createSubscription({customerAccountId:f.customerAccountId.value,planCode:f.planCode.value,salesAgentId:f.salesAgentId.value||undefined,billingCycle:f.billingCycle.value,amount:Number(f.amount.value),maxTenants:Number(f.maxTenants.value||template.maxTenants||1),maxUsers:Number(f.maxUsers.value||template.maxUsers||3),customerSegment:template.segment||'smb',modules:template.modules||[],additionalTenantUnitPriceUsd:Number(template.additionalTenantUnitPriceUsd||0)});Toast.show('Suscripción creada. Ahora vincula los RIF autorizados.','success');await loadCommercial({silent:true});}catch(error){Toast.show(error.message,'error');}});
