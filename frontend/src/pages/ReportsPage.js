import { PageHeader, StatCard, Table, Badge, Button, EmptyState } from '../components/ui/index.js';
...
export const ReportsPage = {
  render(state, { query = {} } = {}) {
    ...
    const purchaseTotal = purchases.reduce((sum,item)=>sum+Number(item.amount||0)+Number(item.iva||0),0);
    const prevPurchaseTotal = prevPurchases.reduce((sum,item)=>sum+Number(item.amount||0)+Number(item.iva||0),0);
    ...
    const rows = activeRows.map((row)=>`<tr><td><strong>${escapeHtml(row.metric)}</strong></td><td class="cg-cell-money">${escapeHtml(row.value)}</td><td>${Badge(row.comparison,row.comparison.startsWith('+')?'success':row.comparison.startsWith('-')?'warning':'brand')}</td><td>${escapeHtml(row.source)}</td><td><button type="button" class="btn btn-secondary !p-2" data-route="${row.route}" aria-label="Abrir detalle"><i class="fa-solid fa-arrow-up-right-from-square"></i></button></td></tr>`);

    return `<section class="cg-page-stack">
      ${PageHeader({eyebrow:'Inteligencia empresarial',title:'Reportes y análisis',description:'Períodos comparables, indicadores financieros, operación, clientes y detalle navegable.',actions:`${Button({id:'btnReportExport',text:'Exportar CSV',icon:'fa-file-csv',variant:'secondary'})}${Button({id:'btnReportPrint',text:'Imprimir',icon:'fa-print',variant:'secondary'})}`})}
      <section class="cgx-toolbar"><strong>Período</strong><label>Desde<input class="input" type="date" data-query-param="dateFrom" value="${query.dateFrom||from.toISOString().slice(0,10)}"></label><label>Hasta<input class="input" type="date" data-query-param="dateTo" value="${query.dateTo||to.toISOString().slice(0,10)}"></label><button type="button" class="btn btn-secondary" data-query-clear="dateFrom,dateTo">Mes actual</button></section>
      <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">${StatCard({label:'Ventas',value:bs(revenue),hint:trend(revenue,prevRevenue),icon:'fa-chart-line'})}${StatCard({label:'Margen',value:bs(grossMargin),hint:`Compras ${bs(purchaseTotal)}`,icon:'fa-sack-dollar',tone:grossMargin>=0?'success':'danger'})}${StatCard({label:'Por cobrar',value:bs(pendingReceivables),icon:'fa-clock',tone:pendingReceivables?'warning':'success'})}${StatCard({label:'Inventario',value:usd(inventory.retailUsd),hint:`${lowStock.length} alertas`,icon:'fa-boxes-stacked'})}</div>
      <nav class="cgx-toolbar" aria-label="Tipo de reporte"><strong>Vista</strong>${[['financial','Finanzas','fa-building-columns'],['operations','Operación','fa-gears'],['customers','Clientes','fa-users']].map(([value,label,icon])=>`<button type="button" class="btn ${tab===value?'btn-primary':'btn-secondary'}" data-report-tab="${value}"><i class="fa-solid ${icon}"></i> ${label}</button>`).join('')}</nav>
      <section class="cgx-section"><header class="cgx-section-head"><div><h2>${tab==='financial'?'Desempeño financiero':tab==='operations'?'Eficiencia operativa':'Concentración de clientes'}</h2><p>Selecciona cualquier indicador para abrir su módulo de origen.</p></div></header><div class="cgx-section-body">${activeRows.length?Table({headers:[{label:'Indicador'},{label:'Valor'},{label:'Comparación'},{label:'Fuente'},{label:'Detalle'}],rows}):EmptyState({title:'Sin datos para el período',description:'Cambia las fechas o registra operaciones.',iconName:'fa-chart-simple'})}</div></section>
    </section>`;
  },
  mount(state,{Toast,UrlStateService}) {
    ...
    document.getElementById('btnReportExport')?.addEventListener('click',()=>{
      const table=document.querySelector('.cgx-table');
      ...
      downloadCsv('reportes-contagest.csv',rows);
