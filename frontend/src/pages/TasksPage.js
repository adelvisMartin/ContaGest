...
export const TasksPage = {
  render(state, { query = {} } = {}) {
    ...
    return `<section class="cg-page-stack">
      ${PageHeader({ eyebrow:'Operaciones', title:'Tareas y seguimiento', description:'Responsables, fechas, dependencias, recurrencias y estados en una vista compartible.', actions:`${Button({id:'btnTaskList',text:'Lista',icon:'fa-list',variant:view==='list'?'primary':'secondary'})}${Button({id:'btnTaskBoard',text:'Tablero',icon:'fa-table-columns',variant:view==='board'?'primary':'secondary'})}` })}
      ...
      <div class="grid gap-4 xl:grid-cols-[380px_minmax(0,1fr)]">
        ...
        <div>${view === 'board' ? `<div class="grid gap-4 xl:grid-cols-4">${columns}</div>` : `<section class="cgx-section"><header class="cgx-section-head"><div><h2>Plan de trabajo</h2><p>Ordenable y filtrado mediante la URL.</p></div></header><div class="cgx-section-body">${Table({headers:[{label:'Tarea'},{label:'Módulo'},{label:'Responsable'},{label:'Inicio'},{label:'Vence'},{label:'Prioridad'},{label:'Estado'},{label:'Acciones'}],rows})}</div></section>`}</div>
      </div>
    </section>`;
  },
  mount(state, { Store, Toast, UrlStateService }) {
    ...
    mountSubmit('#taskForm', (data, form) => {
      Store.update((draft) => {
        draft.tasks = draft.tasks || [];
        draft.tasks.unshift({ id:uid('task'), ...data, createdAt:new Date().toISOString(), updatedAt:new Date().toISOString() });
      });
      form.reset();
      Toast.show('Tarea creada.', 'success');
    });
    qsa('[data-task-status]').forEach((button)=>button.addEventListener('click',()=>Store.update((draft)=>{
      const task=(draft.tasks||[]).find((item)=>item.id===button.dataset.taskStatus);
      if(!task)return;
      const index=STATUSES.indexOf(task.status || 'Pendiente');
      task.status=STATUSES[(index+1)%STATUSES.length];
      task.updatedAt=new Date().toISOString();
    })));
    qsa('[data-delete-task]').forEach((button)=>button.addEventListener('click',()=>{
      Store.update((draft)=>{draft.tasks=(draft.tasks||[]).filter((item)=>item.id!==button.dataset.deleteTask);});
