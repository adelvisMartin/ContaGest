...
export const BankingPage = {
  render(state,{query={}}={}) {
    ...
    ])}${accounts.length?ErpGrid(accountCards,{columns:'four'}):EmptyState({title:'Sin cuentas bancarias',description:'Registra la primera cuenta.',iconName:'fa-building-columns'})}<div class="cg-ui-grid cg-ui-grid-two">${ErpSection({title:'Nueva cuenta',description:'Saldo inicial y moneda de operación.',content:accountForm})}${ErpSection({title:'Nuevo movimiento',description:'El saldo se actualiza en una transacción.',content:movementForm})}</div>${ErpSection({title:'Movimientos',description:'Conciliación auditada e importación controlada.',content:`${toolbar}${table}`})}</section>`;
  },
  mount(state,{Store,Toast,Loading}) {
    ...
    qsa('[data-delete-bank-movement]').forEach((button)=>button.addEventListener('click',async()=>{try{await BankingService.removeMovement(button.dataset.deleteBankMovement);Toast.show('Movimiento eliminado y saldo revertido.','success');await load({silent:true});}catch(error){Toast.show(`No se eliminó el movimiento: ${error.message}`,'error');}}));
