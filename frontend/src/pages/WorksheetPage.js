...
export const WorksheetPage = {
  render(state) {
    ...
    return `<section class="cgx-page accounting-report-page accounting-worksheet-page">
      ${PageHeader({ eyebrow:'Contabilidad', title:'Hoja de trabajo', description:'Integra balance de comprobación, ajustes, balance ajustado, estado de resultados y balance general en una sola hoja.', actions:`${Button({ id:'btnDownloadWorksheet', text:'Descargar CSV', icon:'fa-download' })}${Button({ id:'btnPrintWorksheet', text:'Vista / imprimir', icon:'fa-print', variant:'secondary' })}` })}
      ${MetricGrid([
        { label:'Ajustes registrados', value:String(report.adjustments.length), hint:'No alteran el libro diario', iconName:'fa-sliders', tone:'brand' },
      ...
      ], rows:report.rows }) : EmptyState({ title:'Sin hoja de trabajo', description:'Registra asientos o ajustes para generar la hoja.', iconName:'fa-table-columns' })}
      ${Section({ title:'Resumen de cierre', subtitle:'La utilidad o pérdida se traslada al patrimonio para cuadrar estados.', children:`<div class="cgx-metric-grid"><article class="cgx-metric"><div><p>Ingresos/Egresos</p><strong>${bs(report.totals.incomeCreditLocal - report.totals.incomeDebitLocal)}</strong><small>Resultado del período</small></div></article><article class="cgx-metric"><div><p>Balance general</p><strong>${bs(report.totals.balanceDebitLocal - report.totals.balanceCreditLocal)}</strong><small>Debe - Haber del balance</small></div></article><article class="cgx-metric"><div><p>Control</p><strong>${report.totals.checkLocal === 0 ? 'OK' : 'Revisar'}</strong><small>${bs(report.totals.checkLocal)}</small></div></article></div>` })}
    </section>`;
  },
  mount(state, { Store, Toast }) {
    mountSubmit('#worksheetAdjustmentForm', (data, form) => {
      ...
      Store.update((draft) => { draft.accounting = draft.accounting || {}; draft.accounting.adjustments = [{ id:uid('adj'), ...data, debit, credit, currency:'VES' }, ...(draft.accounting.adjustments || [])]; });
      Toast.show('Ajuste agregado a la hoja de trabajo.', 'success'); form.reset();
    });
    qs('#btnDownloadWorksheet')?.addEventListener('click', () => { const r = buildWorksheet(Store.get()); downloadText('hoja-de-trabajo.csv', rowsToCsv(r.rows, [{label:'Cuenta',key:'account'},{label:'Tipo',key:'type'},{label:'B.Comp Debe',key:'tbDebitLocal'},{label:'B.Comp Haber',key:'tbCreditLocal'},{label:'Ajuste Debe',key:'adjDebitLocal'},{label:'Ajuste Haber',key:'adjCreditLocal'},{label:'Ajustado Debe',key:'adjustedDebitLocal'},{label:'Ajustado Haber',key:'adjustedCreditLocal'},{label:'Resultado Debe',key:'incomeDebitLocal'},{label:'Resultado Haber',key:'incomeCreditLocal'},{label:'Balance Debe',key:'balanceDebitLocal'},{label:'Balance Haber',key:'balanceCreditLocal'}]), 'text/csv;charset=utf-8'); Toast.show('Hoja de trabajo descargada.', 'success'); });
