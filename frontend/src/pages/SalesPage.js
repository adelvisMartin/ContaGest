...
export const SalesPage = {
  render(state) {
    ...
    const total = sales.reduce((s, item) => s + Number(item.amount || 0), 0);
    const pending = sales.filter((item) => String(item.status).toLowerCase().includes('pend')).length;
    const clientOptions = [{ value:'', label:'Consumidor final / sin cliente' }, ...(state.clients || []).map((c) => ({ value:c.id, label:`${c.name} · ${c.rif}` }))];
    const rows = sales.map((sale) => `<tr>
      ...
      <td>${sale.source === 'supabase' ? Badge('Supabase','success') : Badge('Local','warning')}</td>
    ...
    return `<section class="surface rounded-[1.75rem] p-5 sm:p-7">
      ${PageHeader({
        ...
        titleKey:'salesTitle',
        descKey:'salesDesc',
        actions: Button({ id:'btnNewSaleFocus', text:'Nueva venta', icon:'fa-plus', attrs:'type="button"' }) + Button({ id:'btnSyncSales', text:'Sincronizar Supabase', icon:'fa-cloud-arrow-down', variant:'secondary', attrs:'type="button"' })
      })}

      <div class="mb-5 grid gap-4 md:grid-cols-4">
        ${StatCard({label:'Ventas hoy', value:bs(total), hint:'Datos reales al sincronizar', icon:'fa-cash-register'})}
        ${StatCard({label:'Pedidos', value:String(sales.length), hint:`${pending} pendientes`, icon:'fa-cart-shopping', tone:'accent'})}
        ${StatCard({label:'Cobranza pendiente', value:bs(sales.filter((s)=>s.status!=='Cobrada').reduce((a,b)=>a+Number(b.amount||0),0)), icon:'fa-clock'})}
      ...
      <form id="saleForm" class="panel-soft cg-record-form rounded-[1.5rem] p-4">
        ...
        <div class="cg-record-actions">${Button({ text:'Agregar venta', i18n:'add', icon:'fa-cash-register', type:'submit' })}</div>
      </form>

      <div class="panel-soft cg-record-table rounded-[1.5rem] p-4">
        ${Table({ headers:[{key:'invoice'}, {key:'date'}, {key:'client'}, {key:'amount'}, {key:'method'}, {key:'status'}, {label:'Origen'}, {key:'actions'}], rows })}
      </div>
    </section>`;
  },
  mount(state, { Store, Toast, navigate, SupabaseSyncService }) {
    ...
    mountSubmit('#saleForm', async (data, form) => {
      ...
      try {
        ...
        form.reset();
        Toast.show('Venta guardada en Supabase y asiento generado.', 'success');
      } catch (error) {
        Store.update((draft) => { draft.sales = draft.sales || []; draft.sales.unshift({ id:uid('sale'), ...data, amount:Number(data.amount||0), source:'local' }); draft.auditLog.unshift({ id:uid('log'), module:'sales', action:'create-sale-local', at:new Date().toISOString() }); });
        Toast.show(`Venta guardada localmente. Backend: ${error.message}`, 'warning');
      } finally {
    ...
    qsa('[data-send-quote]').forEach((button) => button.addEventListener('click', () => { const sale = (state.sales || []).find((item) => item.id === button.dataset.sendQuote); if (!sale) return; Store.update((draft) => { draft.quote.manualAmount = sale.amount; draft.quote.observation = `Generado desde venta ${sale.invoice}`; }); Toast.show('Venta cargada al cotizador.', 'success'); navigate('cotizacion'); }));
  }
};
