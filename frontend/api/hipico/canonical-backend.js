...
export function buildCanonicalUrl(path, source = process.env) {
  ...
  if (!/^\/api\/v1\/hipico(?:-bot)?(?:\/|\?|$)/.test(value)) {
    throw Object.assign(new Error('HIPICO_CANONICAL_PATH_NOT_ALLOWED'), { code: 'HIPICO_CANONICAL_PATH_NOT_ALLOWED' });
  }
  const url = new URL(value, `${origin}/`);
  if (url.origin !== origin) throw Object.assign(new Error('HIPICO_CANONICAL_PATH_NOT_ALLOWED'), { code: 'HIPICO_CANONICAL_PATH_NOT_ALLOWED' });
  return url.toString();
}
