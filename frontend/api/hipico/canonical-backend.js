...
export function buildCanonicalUrl(path, source = process.env) {
  ...
  if (!origin) throw Object.assign(new Error('HIPICO_CANONICAL_BACKEND_NOT_CONFIGURED'), { code: 'HIPICO_CANONICAL_BACKEND_NOT_CONFIGURED' });
  const value = String(path || '').trim();
  if (!/^\/api\/v1\/hipico(?:-bot)?(?:\/|\?|$)/.test(value)) throw Object.assign(new Error('HIPICO_CANONICAL_PATH_NOT_ALLOWED'), { code: 'HIPICO_CANONICAL_PATH_NOT_ALLOWED' });
...
export async function proxyCanonicalRequest({ path, method = 'GET', headers = {}, body = undefined, source = process.env }) {
  ...
  if (Number.isFinite(declared) && declared > MAX_PROXY_RESPONSE_BYTES) throw Object.assign(new Error('HIPICO_CANONICAL_RESPONSE_TOO_LARGE'), { code: 'HIPICO_CANONICAL_RESPONSE_TOO_LARGE' });
  const bytes = Buffer.from(await response.arrayBuffer());
  if (bytes.byteLength > MAX_PROXY_RESPONSE_BYTES) throw Object.assign(new Error('HIPICO_CANONICAL_RESPONSE_TOO_LARGE'), { code: 'HIPICO_CANONICAL_RESPONSE_TOO_LARGE' });
  return { status: response.status, contentType: String(response.headers.get('content-type') || 'application/json; charset=utf-8'), retryAfter: response.headers.get('retry-after'), requestId: response.headers.get('x-request-id'), body: bytes };
}
