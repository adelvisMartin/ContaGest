...
test('#157 closes only after the measured gate passes and #155 is closed',()=>{
  const gateIndex=workflow.indexOf('erp-performance-gate-v157.mjs check');
  const issue155Index=workflow.indexOf('gh issue view 155');
  const closeIndex=workflow.indexOf('gh issue close 157');
  assert.ok(gateIndex>=0&&issue155Index>gateIndex&&closeIndex>issue155Index);
});
