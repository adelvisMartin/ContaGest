...
test('v17 DRIFT without verified restore evidence is BLOCKED',async()=>{
  ...
  const plan=buildRolloutPlan({
    ...
    driftReport:drift(),
    migrationUnits:units,
    backupEvidence:validBackup({restoreVerified:false}),
    changeTicket:'#375',
    now:new Date(NOW)
  });
...
test('v17 stale backup evidence is rejected fail-closed',()=>{
  const result=validateBackupEvidence(validBackup({checkedAt:'2026-09-16T00:00:00.000Z'}),{
    candidateSha:SHA,
    targetDatabase:'postgres',
    now:new Date(NOW),
    maxAgeHours:24
