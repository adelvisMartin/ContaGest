...
test('infrastructure blocker is emitted only when at least one required job is runner-blocked', () => {
  assert.equal(summarizeBlocker(['PASS', 'PASS', 'NOT_EXECUTED']), '');
  assert.equal(summarizeBlocker(['PASS', 'BLOCKED', 'FAIL']), 'BLOCKED_INFRASTRUCTURE');
});
