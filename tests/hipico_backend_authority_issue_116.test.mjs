...
test('RLS hardening is forced and restrictive without embedding service-role credentials', async () => {
  const [migration, config, client] = await Promise.all([
    fs.readFile(migrationPath, 'utf8'), fs.readFile(configPath, 'utf8'), fs.readFile(supabasePath, 'utf8')
  ]);
  ...
  assert.doesNotMatch(config + client, /SUPABASE_SERVICE_ROLE_KEY|service_role|sb_secret_/i);
});
