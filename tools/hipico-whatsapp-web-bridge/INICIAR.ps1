...
if (($EnableLabSend -or $EnableLabInput) -and -not $bindings) {
  Fail "LAB automático requiere IDs @g.us verificados. Ejecuta primero: .\INICIAR.ps1 -CaptureGroupIds"
}
...
if ($EnableLabSend) {
  ...
  Write-Host 'ENVÍO LAB deshabilitado. Observación/clasificación solamente.' -ForegroundColor Green
}
