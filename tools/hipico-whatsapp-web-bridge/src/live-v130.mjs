...
// Si el backfill histórico ya dejó IDs persistidos, no hacemos otro baseline:
// cualquier mensaje visible que no esté en seen debe procesarse como nuevo.
let sourceBaselineCompleted = seen.size > 0;
...
async function loadSeen() {
  for (const file of [SEEN_FILE, LEGACY_SEEN_FILE]) {
    try {
      ...
      if (Array.isArray(data)) return new Set(data.filter(Boolean).slice(-250000));
    } catch {}
  }
  return new Set();
}
async function saveSeenNow() {
  const values = [...seen].slice(-250000);
...
async function queueEvent(event) { return spoolJson(EVENT_SPOOL_DIR, `${event.channelKey}|${event.externalMessageId}`, event); }
async function queueMirror(mirror) { return spoolJson(MIRROR_SPOOL_DIR, mirror.sourceExternalMessageId, mirror); }
...
async function openGroup(match, exact = true) {
  ...
  if (await clickVisibleGroupCandidate(match, exact) && await currentChatMatches(match, exact)) return true;
  if (await openViaSearch(match, exact) && await currentChatMatches(match, exact)) return true;
  return false;
}
...
async function extractVisibleMessages() {
  return page.evaluate(() => {
    ...
    for (const node of candidates) {
      ...
      const mediaName = Array.from(node.querySelectorAll?.('[title], [aria-label]') || [])
      ...
      rows.push({ id, pre, text, fromMe, hasMedia, mediaKind, mediaName, senderLabel });
      used.add(id);
    }
    return rows;
  });
...
function rowToSourceEvent(row) {
  ...
  return {
    ...
    mediaName: String(row.mediaName || '').slice(0, 240),
...
async function sendMirrorToLab(mirror) {
  ...
  await sleep(800);
  if (!(await visibleLabHasTag(mirror.mirrorTag))) throw new Error('No pude verificar visualmente el mensaje shadow en el grupo lab.');
...
async function main() {
  console.log('\n========================================================');
  console.log(' Control Hipico - WhatsApp Web Bridge v1.3.0');
