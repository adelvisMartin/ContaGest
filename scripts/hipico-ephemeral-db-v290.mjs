...
function fail(message) {
  console.error(`[hipico-v290] ${message}`);
  process.exitCode = 1;
...
async function createDatabase() {
  ...
  const state = { databaseName, host: parsed.hostname, port: parsed.port || '5432', status: 'planned', createdAt: null, runId: process.env.GITHUB_RUN_ID || process.env.HIPICO_E2E_RUN_ID || 'local' };
  await writeState(state);

  const client = new Client({ connectionString: adminUrl });
  await client.connect();
  try { await client.query(`CREATE DATABASE ${quoteIdentifier(databaseName)}`); }
  ...
  await writeGithubEnv('DIRECT_DATABASE_URL', databaseUrl.toString());
...
async function dropDatabase() {
  ...
  if (!databaseName) {
    try { databaseName = String(JSON.parse(await fs.readFile(stateFile, 'utf8')).databaseName || '').trim(); }
    catch { console.log('[hipico-v290] no ephemeral database state exists; nothing to clean'); return; }
  }
  quoteIdentifier(databaseName);
  const client = new Client({ connectionString: adminUrl });
  await client.connect();
  ...
  try { const saved = JSON.parse(await fs.readFile(stateFile, 'utf8')); await writeState({ ...saved, status: 'dropped', droppedAt: new Date().toISOString() }); } catch {}
