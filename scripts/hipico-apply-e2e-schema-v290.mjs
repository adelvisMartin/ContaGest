...
async function expectPermissionDenied(client, role, subject, sql) {
  ...
  await client.query('ROLLBACK');
  throw new Error(`RBAC statement unexpectedly succeeded for ${role}: ${sql.slice(0, 100)}`);
}
...
try {
  ...
  for (const name of [
    ...
    if (!triggers.has(name)) throw new Error(`Immutable audit trigger missing: ${name}`);
  }
  ...
  await expectPermissionDenied(client, 'anon', null,
    `SELECT owner_id FROM public.hipico_workspaces LIMIT 1`);
  await expectPermissionDenied(client, 'anon', null,
    `SELECT id FROM public.hipico_documents LIMIT 1`);
  ...
  console.log(`[hipico-v290] current schema ready (${required.length} required tables; v12-v21 RLS/RBAC evidence executed)`);
} catch (error) {
  try { await client.query('ROLLBACK'); } catch {}
  throw error;
} finally {
