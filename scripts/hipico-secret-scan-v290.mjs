...
function git(args) {
  return execFileSync('git', args, { cwd: root, encoding: 'utf8', windowsHide: true, maxBuffer: 16 * 1024 * 1024 }).trim();
}

function expectedCandidateSha() {
  const value = String(process.env.HIPICO_CANDIDATE_SHA || process.env.HIPICO_QA_SHA || process.env.HIPICO_RELEASE_SHA || process.env.GITHUB_SHA || git(['rev-parse', 'HEAD'])).trim();
...
function changedFiles(base, candidate) {
  if (SHA40.test(base) && base.toLowerCase() !== candidate.toLowerCase()) {
    return git(['diff', '--name-only', '--diff-filter=ACMRTUXB', `${base}..${candidate}`, '--']).split(/\r?\n/).filter(Boolean);
  }
...
function scanJwtServiceRole(source, file, findings) {
  ...
  for (const match of source.matchAll(jwt)) {
    try {
      const payloadText = Buffer.from(match[0].split('.')[1].replace(/-/g, '+').replace(/_/g, '/'), 'base64').toString('utf8');
      const payload = JSON.parse(payloadText);
      if (String(payload?.role || '').toLowerCase() === 'service_role') findings.push({ rule: 'SUPABASE_SERVICE_ROLE_JWT', file, line: lineNumber(source, match.index || 0) });
    } catch {}
  }
}
...
function scanRemoteCredentialUrl(source, file, findings) {
  ...
  for (const match of source.matchAll(urlPattern)) {
    ...
    if (!['localhost', '127.0.0.1', '::1'].includes(host)) findings.push({ rule: 'REMOTE_DATABASE_EMBEDDED_CREDENTIAL', file, line: lineNumber(source, match.index || 0) });
  }
}
...
for (const relative of files) {
  ...
  for (const rule of RULES) {
    ...
    for (const match of source.matchAll(rule.pattern)) findings.push({ rule: rule.id, file: relative, line: lineNumber(source, match.index || 0) });
  }
...
const report = {
  schema: 'hipico-secret-scan.v290',
  sha: candidate,
...
if (findings.length) {
  for (const item of findings) console.error(`[hipico-secret-scan] ${item.rule} ${item.file}:${item.line}`);
