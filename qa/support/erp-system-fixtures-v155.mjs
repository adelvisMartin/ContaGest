...
const samples={
  client:{id:'qa-client-1',name:'Cliente QA',displayName:'Cliente QA',rif:'J-00000000-1',email:'cliente@example.test',phone:'0412-0000000',status:'active'},
  product:{id:'qa-product-1',sku:'QA-001',name:'Producto QA',description:'Producto sintético',stock:25,quantity:25,price:125.5,cost:90.25,status:'active'},
  sale:{id:'qa-sale-1',number:'V-QA-0001',customerId:'qa-client-1',customerName:'Cliente QA',status:'open',subtotal:100,total:116,tax:16,date:'2026-09-03'},
  purchase:{id:'qa-purchase-1',number:'C-QA-0001',supplierId:'qa-supplier-1',supplierName:'Proveedor QA',status:'open',subtotal:100,total:116,tax:16,date:'2026-09-03'},
  bank:{id:'qa-bank-1',name:'Banco QA',accountNumber:'****0001',currency:'USD',balance:12345.67,status:'active'},
  employee:{id:'qa-employee-1',fullName:'Empleado QA',email:'empleado@example.test',status:'active',salary:500},
  patient:{id:'qa-patient-1',kind:'human',displayName:'Paciente QA',firstName:'Paciente',lastName:'QA',email:'paciente@example.test',phone:'0412-0000001'},
  appointment:{id:'qa-appointment-1',patientId:'qa-patient-1',patientName:'Paciente QA',date:'2026-09-03',time:'10:30',durationMinutes:50,status:'scheduled',reason:'Seguimiento QA'},
  task:{id:'qa-task-1',title:'Tarea QA',status:'open',priority:'high',dueDate:'2026-09-04'},
  audit:{id:'qa-audit-1',action:'QA_SYNTHETIC',actor:'qa@example.test',createdAt:now,entity:'fixture'},
  ledger:{id:'qa-ledger-1',date:'2026-09-03',account:'1.1.01',debit:100,credit:0,description:'Asiento QA'},
...
function mutationResult(pathname,body){
  const base=Array.isArray(dataForPath(pathname))?dataForPath(pathname)[0]:dataForPath(pathname);
  return {...base,...(body&&typeof body==='object'?body:{}),id:body?.id||base?.id||'qa-created-1',updatedAt:now};
}
...
export function fixtureForRequest({url,method='GET',state='baseline',body=null}){
  ...
  return {status:200,delayMs,body:{ok:true,data}};
}
