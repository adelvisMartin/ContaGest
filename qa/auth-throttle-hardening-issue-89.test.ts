...
test('issue #89 hardening: enumeration parity and retention without successful login',async(t)=>{
  assert.equal(String(process.env.NODE_ENV||'').toLowerCase(),'test');

  const tenant=await prisma.tenant.create({
    data:{rif:RIF,name:`${RUN} Tenant`,legalName:`${RUN} Tenant`,plan:'qa',status:'active',settings:{}}
  });
  const user=await prisma.userProfile.create({
    data:{tenantId:tenant.id,email:EMAIL,fullName:'QA Auth 89 Hardening',passwordHash:await bcrypt.hash(PASSWORD,12),status:'active'}
  });
  ...
  t.after(async()=>{
    ...
    await prisma.userProfile.deleteMany({where:{id:user.id}}).catch(()=>undefined);
  ...
  // This is intentionally a coarse guard, not a brittle microbenchmark. Every
  // non-throttled invalid-credential path must execute non-trivial password work.
