...
async function runtimeControlAudit(page,route){
  return page.evaluate((activeRoute)=>{
    ...
    for(const button of buttons){
      ...
      if(type==='button'&&!bound.includes('click')&&!routeControl&&!declarativeData&&!reactMui){
        buttonFindings.push({kind:'no-runtime-click-contract',label,id:button.id||'',html:button.outerHTML.slice(0,180)});
      }
      if(type==='submit'&&!submitBound&&!reactMui){
        buttonFindings.push({kind:'submit-form-unbound',label,formId:form?.id||'',html:button.outerHTML.slice(0,180)});
      }
    }
    ...
    for(const node of icons){
      ...
      const usedFamilies=families.filter((family)=>classes.includes(family));
      const glyphs=classes.filter((value)=>value.startsWith('fa-')&&!families.includes(value));
      ...
      if(parentButton&&visible(parentButton)&&!text(parentButton)&&!parentButton.getAttribute('aria-label')&&!parentButton.getAttribute('title')){
        iconFindings.push({kind:'icon-only-button-unlabelled',classes:classes.join(' '),button:parentButton.outerHTML.slice(0,180)});
      }
    }
...
test('all 58 runtime routes expose bound, labelled controls and valid icon families',async({page})=>{
  ...
  for(const item of MODULE_VISUAL_CATALOG){
    ...
    try{
      ...
      buttonCount+=audit.buttons;
      iconCount+=audit.icons;
      if(audit.buttonFindings.length||audit.iconFindings.length||audit.duplicates.length||pageErrors.length){
        failures.push({...audit,pageErrors});
      }
    }catch(error){
      failures.push({route:item.route,error:String(error?.message||error),pageErrors});
    }finally{
      page.off('pageerror',listener);
    }
  }
  console.log(`[browser-control-v16] rutas=${MODULE_VISUAL_CATALOG.length} botones-visibles=${buttonCount} iconos-visibles=${iconCount} fallos=${failures.length}`);
