...
test('licensed client must explicitly accept current legal documents and necessary cookies before continuing',async({page})=>{
  ...
  await page.addInitScript(()=>{
    ...
    localStorage.setItem('contagest_auto_sync_enabled','false');localStorage.setItem('contagest_analytics_backend_enabled','false');
  });
  ...
  await page.route('**/api/v1/legal/accept',async(route)=>{acceptedBody=route.request().postDataJSON();await route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true,data:{accepted:true,cookiePreferences:{necessaryAcknowledged:true,analyticsEnabled:false,marketingEnabled:false}}})});});
  ...
  const dialog=page.getByRole('dialog',{name:/Antes de continuar en ContaGest/i});await expect(dialog).toBeVisible();
  ...
  const accept=page.locator('#cgLegalAccept');await expect(accept).toBeDisabled();
  for(const checkbox of await page.locator('[data-legal-document]').all())await checkbox.check();
  await page.locator('#cgNecessaryCookies').check();await expect(accept).toBeEnabled();await accept.click();
  await expect(dialog).toHaveCount(0);expect(acceptedBody.necessaryCookiesAcknowledged).toBe(true);expect(acceptedBody.analyticsCookies).toBe(false);expect(acceptedBody.documents).toHaveLength(5);
