...
async function openSidebar(page){
  if(!(await page.locator('body').evaluate((body)=>body.classList.contains('cg-menu-open'))))await page.locator('#btnOpenSidebar').click();
  await expect(page.locator('body')).toHaveClass(/cg-menu-open/);
}

test('every actual sidebar route button navigates to the requested module on mobile',async({page})=>{
  await seed(page);
  ...
  for(const route of routes){
    try{
      ...
      const current=new URL(page.url()).searchParams.get('module');
      expect(current||route).toBe(route);
    }catch(error){failures.push({route,error:String(error?.message||error)});}
  }
  console.log(`[mobile-navigation-v163] sidebar-routes=${routes.length} fallos=${failures.length}`);
  expect(failures,JSON.stringify(failures,null,2)).toEqual([]);
});

test('command palette opens, filters and navigates from a touch viewport',async({page})=>{
  await seed(page);
  await dashboard(page);
  ...
  await expect.poll(()=>page.locator('body').getAttribute('data-route')).toBe(target);
});
