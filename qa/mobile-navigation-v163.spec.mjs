...
async function dashboard(page){await page.goto('/?module=dashboard',{waitUntil:'domcontentloaded'});await page.waitForSelector('#pages',{state:'attached',timeout:20_000});}
...
test('every actual sidebar route button navigates to the requested module and closes the drawer on mobile',async({page})=>{
  ...
  for(const route of routes){
    ...
    }catch(error){failures.push({route,error:String(error?.message||error),url:page.url()});}
  }
