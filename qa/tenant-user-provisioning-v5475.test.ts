...
test('54/75 real PostgreSQL: create-only tenant registration and atomic licensed-user provisioning',async(t)=>{
  const harness=await createRealBackendHarness();
  const run=randomUUID().slice(0,8);
  const tenantRif=`QA54-${run}`.toUpperCase();
  const tenantName=`QA54 Tenant ${run}`;
  const adminEmail=`qa54-admin-${run}@example.test`;
  const duplicateEmail=`qa54-duplicate-${run}@example.test`;
  const clientEmail=`qa54-client-${run}@example.test`;
  ...
  await t.test('register creates tenant admin role and membership as one usable identity',async()=>{
    ...
    const result=await harness.request('/auth/register',{
      method:'POST',
      body:JSON.stringify({
        tenantRif,tenantName,legalName:`${tenantName} Legal`,fullName:'QA54 Admin',email:adminEmail,
  ...
  await t.test('license endpoint atomically materializes user membership RBAC and license linkage',async()=>{
    const data=await harness.ok('/licenses',{
      method:'POST',
      body:JSON.stringify({
        userEmail:clientEmail,fullName:'QA54 Dentistry User',plan:'trial',days:7,
