...
function q(values,p){
  assert.ok(Array.isArray(values)&&values.length>0,'ERP157_PERCENTILE_SAMPLES_REQUIRED');
  assert.ok(values.every((value)=>typeof value==='number'&&Number.isFinite(value)&&value>=0),'ERP157_PERCENTILE_SAMPLES_INVALID');
...
async function waitForRouteReady(page,route){
  if(route==='login'){
    await page.waitForSelector('.login-shell',{state:'visible'});
    return;
  }
  await page.waitForSelector('#app',{state:'attached'});
  await page.waitForFunction((expected)=>document.body.getAttribute('data-route')===expected,route);
}
async function gotoRoute(page,route){const started=performance.now();await page.goto(`/?module=${route}`,{waitUntil:'domcontentloaded'});await waitForRouteReady(page,route);return performance.now()-started;}
async function navigateInApp(page,route){
  const target=page.locator(`[data-route="${route}"]:visible`).first();
...
test('issue #157 frontend measured performance evidence',async({page})=>{
  ...
  const routeSwitch=[];for(let i=0;i<7;i++){await gotoRoute(page,'dashboard');routeSwitch.push(await navigateInApp(page,'clientes'));}
  ...
  await page.evaluate(()=>{window.__cgLongTasks=[];});
  const heapBefore=await heapUsed(cdp);const sessionStarted=performance.now();
  for(let i=0;i<30;i++)await navigateInApp(page,['clientes','reportes','dashboard'][i%3]);
